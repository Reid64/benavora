...
6. A migration that rule 4 forbids any agent from applying, and that a human has decided to hold open, is recorded as a **named deferral** in `DEFERRED_MIGRATIONS` in `scripts/audit/forge-gates/work-landed.mjs` — never left to fail the gate indefinitely, and never quietly filtered out. An entry must name the file exactly plus `owner`, `since`, `decision` (pointing at the directive text that records it) and `reason`. The gate prints every deferral on every run, pass or fail, and `partitionDeferred()` audits the registry in both directions: an entry whose file has left disk, or whose migration has since been applied, **fails check 3**. Adding an entry is a governance act — no entry without a recorded human decision, and a deferral covers exactly one file, never a pattern.

**Update (AR-17.1 recovery, 2026-09-19):** rule 6 and the registry were added because the rule-1/rule-4 deadlock was no longer theoretical: with check 3 finally asserting, `170` alone failed the last gate of every downstream queue — on a condition already decided, already documented, and that no agent is permitted to act on. Rule 5 named the deadlock; it had no mechanism, so the standing outcome was a permanently red gate, which is the state that stops being read (see the AR-10.3 incident in the `work-landed.mjs` header). `170` is now the registry's one entry. Gate run 2026-09-19: ledger read via service_role, 202 rows, 1 deferral printed, exit 0. Self-test: **10 clean-pass, 9 catch, 0 unexpected** — cases 10a–10e added, covering deferred-does-not-fail, genuine drift beside a deferral still caught, stale entry caught, applied-migration entry caught, and an audit of the shipped registry against the real `supabase/migrations/` directory.
...
`170_pil_prospects_auto_research_run_trigger.sql` **remains open and is still Reid's call.** It was applied during that pass and then fully reverted (trigger dropped, function dropped, ledger row deleted, confirmed zero `pil_research_runs` rows created in the interim) once rule 4 was re-read: its AFTER INSERT trigger creates a `pil_research_runs` row per new prospect, and `/api/cron/pil-research` polls those every 10 minutes and spends real Anthropic budget on each. Nothing in `src/` or `worker/` depends on the trigger existing, so leaving it unapplied breaks no code. Ledger is at 202 rows. *(Superseded in mechanism only by rule 6 above: the decision is unchanged and 170 is still unapplied — it is now a named deferral that the gate prints every run, rather than a hard failure that blocked every downstream queue.)*

**Enforcement:** `scripts/audit/forge-gates/work-landed.mjs` check 3; `scripts/audit/forge-gates/work-landed.self-test.mjs` cases 7–9 cover all three ledger conventions and the duplicate-stem case, cases 10a–10e cover the deferral registry.

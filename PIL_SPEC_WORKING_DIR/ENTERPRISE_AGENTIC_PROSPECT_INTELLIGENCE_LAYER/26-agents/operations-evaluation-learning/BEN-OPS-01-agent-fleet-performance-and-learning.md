# BEN-OPS-01 — Agent Fleet Performance and Learning Agent

**Specification ID:** PIL-AGENT-BEN-OPS-01  
**Version:** 1.0.0  
**Family:** operations-evaluation-learning  
**Default autonomy:** A1_A2  
...
Evaluate fleet outcomes, quality, calibration, cost, latency, failure, recovery, critic overturns, and downstream results; propose reversible versioned improvements without self-modifying production policy or code.
...
`BEN_OPS_01Goal.v1` MUST include:
...
- **Agent Performance**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Evaluation Results**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Outcome Feedback**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Cost/Latency**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Error Taxonomy**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Learning Proposals**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
...
- `BEN-SUP-01` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-SUP-04` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
...
- `BEN-SUP-06` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
...
- `PIL-SVC-28` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-30` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
...
`BEN_OPS_01Decision.v1` requires:
...
`A1_A2` is the default ceiling, not a self-grant. Human review is mandatory when tenant policy requires it, material ambiguity remains above threshold, a legal/ethical representation is requested, sensitive data use is uncertain, or the action would exceed reversible/research authority.
...
- `AGENT_PERFORMANCE`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `EVALUATION_RESULTS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `OUTCOME_FEEDBACK`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `COST/LATENCY`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `ERROR_TAXONOMY`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `LEARNING_PROPOSALS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
...
This document completes the specification for `BEN-OPS-01`. It does **not** claim runtime implementation, executed tests, live-provider validation, a production score, or production readiness.

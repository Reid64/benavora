...
**Specification ID:** PIL-AGENT-BEN-REL-03  
**Version:** 1.0.0  
**Family:** relationship  
**Default autonomy:** A2  
...
Map evidence-backed corporate, employment, ownership, vendor, partnership, and governance connections relevant to a prospect without collapsing organizational relationships into personal relationships.
...
`BEN_REL_03Goal.v1` MUST include:
...
- **Company Legal Identity**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Role/Ownership Linkage**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Subsidiary/Parent Boundaries**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Shared Employment Interval**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Commercial Versus Philanthropic Context**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Decision-Authority Uncertainty**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
...
- `BEN-REL-05` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
...
`BEN_REL_03Decision.v1` requires:
...
- `COMPANY_LEGAL_IDENTITY`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `ROLE/OWNERSHIP_LINKAGE`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `SUBSIDIARY/PARENT_BOUNDARIES`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `SHARED_EMPLOYMENT_INTERVAL`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `COMMERCIAL_VERSUS_PHILANTHROPIC_CONTEXT`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `DECISION-AUTHORITY_UNCERTAINTY`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
...
This document completes the specification for `BEN-REL-03`. It does **not** claim runtime implementation, executed tests, live-provider validation, a production score, or production readiness.

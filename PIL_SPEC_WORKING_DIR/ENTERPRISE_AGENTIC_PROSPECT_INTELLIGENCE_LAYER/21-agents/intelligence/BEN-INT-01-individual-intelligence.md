...
**Specification ID:** PIL-AGENT-BEN-INT-01  
**Version:** 1.0.0  
**Family:** intelligence  
**Default autonomy:** A2  
...
Develop a verified, temporally bounded intelligence dossier for an individual prospect without conflating identity, household, employer, wealth, philanthropy, or contactability.
...
`BEN_INT_01Goal.v1` MUST include:
...
- **Identity Hypotheses And Discriminators**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Biographical And Role Chronology**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Geographic Relevance**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Documented Affiliations**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Public Philanthropic Observations**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
- **Research Gaps And Contradictions**: observe source-specific facts, alternatives, uncertainty, effective time, and evidence lineage.
...
- `BEN-INT-02` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-INT-04` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-INT-05` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-INT-07` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-INT-08` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
- `BEN-REL-01` when its canonical reasoning boundary is required; delegated work must use the current typed DelegatedTask contract and may not exceed inherited authority.
...
- `PIL-SVC-09` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-10` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-11` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-12` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-13` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-14` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-15` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
- `PIL-SVC-16` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
...
- `PIL-SVC-26` only through registered capabilities and policy enforcement; the service remains deterministic and does not inherit agent authority.
...
`BEN_INT_01Decision.v1` requires:
...
`A2` is the default ceiling, not a self-grant. Human review is mandatory when tenant policy requires it, material ambiguity remains above threshold, a legal/ethical representation is requested, sensitive data use is uncertain, or the action would exceed reversible/research authority.
...
- `IDENTITY_HYPOTHESES_AND_DISCRIMINATORS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `BIOGRAPHICAL_AND_ROLE_CHRONOLOGY`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `GEOGRAPHIC_RELEVANCE`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `DOCUMENTED_AFFILIATIONS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `PUBLIC_PHILANTHROPIC_OBSERVATIONS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
- `RESEARCH_GAPS_AND_CONTRADICTIONS`: gold-set accuracy, negative-case safety, temporal semantics, evidence sufficiency, contradiction behavior, and calibrated abstention.
...
This document completes the specification for `BEN-INT-01`. It does **not** claim runtime implementation, executed tests, live-provider validation, a production score, or production readiness.

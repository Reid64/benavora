...
exports.SearchProfileOptimizerAgent = void 0;
...
class SearchProfileOptimizerAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async loadDiscoveredScores(profile, sinceIso) {
    ...
    async run(triggerSource) {
        ...
        try {
            const [{ data: profileRows, error: profilesError }, { data: orgRow }] = await Promise.all([
                this.supabase
            ...
            if (profilesError) {
                throw new Error(`Failed to load search profiles: ${profilesError.message}`);
            }
            const profiles = (profileRows ?? []);
            const orgMission = orgRow?.mission_statement ??
            ...
            for (const profile of profiles) {
                try {
                    ...
                    const highScoreCount = scores.filter((s) => (s.eligibility_score ?? 0) >= HIGH_SCORE_THRESHOLD).length;
                    const hitRate = totalDiscovered > 0 ? highScoreCount / totalDiscovered : 0;
                    const underperforming = hitRate < UNDERPERFORMING_HIT_RATE || totalDiscovered === 0;
                    if (!underperforming)
                        continue;
                    profilesFlagged++;
                    const response = await (0, claude_1.callClaude)({
                        maxTokens: 300,
                        prompt: `This nonprofit search profile has underperformed. Keywords: ${profile.keywords.join(", ")}. ` +
                            `Org mission: ${orgMission}. Suggest 5 improved keyword additions to find better-matched grant opportunities.`,
                    });
                    tokensUsed += response.usage.totalTokens;
                    await this.createNotification("search_profile_optimization", `Search profile "${profile.name}" is underperforming`, `Hit rate ${(hitRate * 100).toFixed(0)}% over ${totalDiscovered} discovered opportunit${totalDiscovered === 1 ? "y" : "ies"} in the last ${DISCOVERY_WINDOW_DAYS} days. Suggested keyword additions: ${response.text.trim()}`, {
...
exports.SearchProfileOptimizerAgent = SearchProfileOptimizerAgent;

...
class DeadlineExtractor extends base_agent_1.BaseAgent {
    agentType = "deadline_extraction";
    async execute(input) {
        const opportunityId = input.opportunityId;
        const { data: opp, error } = await this.client
            .from("opportunities")
            .select("id, name, deadline, recurrence, description, eligibility_requirements")
            .eq("id", opportunityId)
        ...
        const name = opp.name;
        const deadlineRaw = opp.deadline ?? null;
        ...
        const text = `${opp.description ?? ""} ${opp.eligibility_requirements ?? ""}`.toLowerCase();
        if (/\breport(ing|s)?\b/.test(text)) {
            candidates.push({
                ...
                description: "Estimated reporting deadline derived from a reporting requirement in the opportunity - confirm the actual date.",
            });
        }
        // Renewal one year out for annually recurring opportunities.
        if (opp.recurrence === "annual") {
            candidates.push({
        ...
        const { data: existingRows } = await this.client
            ...
            .eq("organization_id", this.organizationId);
        const existing = new Set((existingRows ?? []).map((r) => `${r.deadline_type}|${r.due_date}`));
        const toInsert = candidates.filter((c) => !existing.has(`${c.deadline_type}|${c.due_date}`));
        if (toInsert.length > 0) {
            const { error: insertError } = await this.client.from("deadlines").insert(toInsert.map((c) => ({
            ...
            if (insertError) {
                throw new base_agent_1.AgentError("Failed to create deadlines.", "write_failed");
            }
        }
        const created = toInsert.map((c) => ({
...
exports.DeadlineExtractor = DeadlineExtractor;

...
exports.BANNED_VOCABULARY = void 0;
exports.stripMarkdown = stripMarkdown;
exports.stripEmDashes = stripEmDashes;
exports.replaceAiVocabulary = replaceAiVocabulary;
exports.enforceHumanization = enforceHumanization;
exports.analyzeHumanization = analyzeHumanization;
...
function stripMarkdown(text) {
    ...
    out = out.replace(/^#{1,6}[ \t]+(.+?)[ \t]*#*$/gm, (_m, heading) => {
        count++;
        return heading.trim();
    });
    // Bold: **text** and __text__
    out = out.replace(/\*\*([^*]+)\*\*/g, (_m, inner) => { count++; return inner; });
    out = out.replace(/__([^_]+)__/g, (_m, inner) => { count++; return inner; });
    ...
    out = out.replace(/(?<!\*)\*([^*\n]+)\*(?!\*)/g, (_m, inner) => { count++; return inner; });
    out = out.replace(/(?<!_)_([^_\n]+)_(?!_)/g, (_m, inner) => { count++; return inner; });
    // Horizontal rules (--- / *** / ___)
    out = out.replace(/^[ \t]*(?:[*\-_][ \t]*){3,}[ \t]*$/gm, () => { count++; return ""; });
    // Blockquotes: > text → text
    out = out.replace(/^[ \t]*>[ \t]?(.*)$/gm, (_m, inner) => { count++; return inner; });
...
function fixSentenceCase(text) {
    return text
        .replace(/^(\s*)([a-z])/, (_m, lead, ch) => lead + ch.toUpperCase())
        .replace(/([.!?]\s+|\n[ \t]*)([a-z])/g, (_m, sep, ch) => sep + ch.toUpperCase());
}
 ...
 * model returned.
 */
function enforceHumanization(text) {
...
function analyzeHumanization(text) {
    ...
    for (const n of starterCounts.values()) {
        if (n > 1)
            repeatedStarters += n - 1;
    }
    // Inline colon lists: a colon mid-sentence followed by a comma-separated run.
    const colonListPatterns = countMatches(text, /[A-Za-z]+\s*:\s+[^.:\n]*,[^.:\n]*,[^.:\n]*/g);
    const contractions = countMatches(text, /\b[A-Za-z]+['’](?:s|re|ve|ll|d|t|m)\b|\b[A-Za-z]+n['’]t\b/gi);
    let bannedVocabHits = 0;
    for (const { pattern } of BANNED_PHRASES)
        bannedVocabHits += countMatches(text, pattern);
    ...
    return {
        ...
        hasDramaticVariance: shortest > 0 && shortest <= SHORT_SENTENCE_MAX && longest >= LONG_SENTENCE_MIN,
        repeatedStarters,
        colonListPatterns,
        contractions,
        bannedVocabHits,
 ...
 */
function computeHumanizationScore(metrics) {
    let score = 100;
    ...
    if (!metrics.hasDramaticVariance)
        score -= 14;
    if (metrics.burstiness < 0.35 && metrics.sentenceCount >= 4)
        score -= 12;
    // Stylistic tells.
    if (metrics.repeatedStarters > 2)
        score -= (metrics.repeatedStarters - 2) * 3;
    score -= metrics.colonListPatterns * 4;
    if (metrics.contractions === 0 && metrics.sentenceCount >= 5)
        score -= 8;
    return Math.max(0, Math.min(100, Math.round(score)));
}
function renderOrgFacts(org) {
    if (!org) {
    ...
    add("Annual budget", org.annualBudget != null
...
function renderKnowledgeFacts(entries) {
    if (entries.length === 0) {
...
function renderVoiceSamples(narratives) {
 ...
 */
function buildHumanizerPrompt(context) {
    ...
    const system = [
        ...
        `- AI VOCABULARY: never use these words/phrases - ${exports.BANNED_VOCABULARY.join(", ")}. Replace them with plain, specific language.`,
 ...
 */
async function runHumanizer(input) {
    ...
    const response = await (0, claude_1.callClaude)({
        system,
        prompt,
        model: input.model ?? claude_1.DEFAULT_MODEL,
        maxTokens: input.maxTokens ?? claude_1.DEFAULT_MAX_TOKENS,
        temperature: 0.9,
    });

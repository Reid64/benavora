...
exports.sendMorningDigest = sendMorningDigest;
/**
 ...
 * both are safe.
 */
async function sendMorningDigest(orgId, supabase) {
    const now = new Date();
    const since = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    const horizon = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000);
    const [discoveryRes, reputationRes, recommendationsRes, deadlinesRes] = await Promise.all([
        supabase
    ...
    const totalItems = counts.newOpportunities + counts.funderAlerts + counts.upcomingDeadlines;
    if (totalItems === 0)
        return;
    const message = `Your Morning Funding Brief: ${counts.newOpportunities} new ` +

...
class DeadlinePredictionAgent extends base_agent_1.BaseAgent {
    ...
    async execute(input) {
        ...
        if (error) {
            throw new base_agent_1.AgentError("Failed to load historical deadlines.", "db_error");
        }
        const validRows = (rows ?? []).filter((r) => typeof r.deadline === "string" && r.deadline.trim() !== "");
        ...
        const predictions = detectPatterns(validRows.map((r) => r.deadline), nowMs, cutoffMs);
        let createdCount = 0;
        for (const pred of predictions) {
            ...
            if (created?.id) {
                ...
                pred.createdOpportunityId = created.id;
                createdCount++;
            }
        }
        const categoryLabel = category ? ` for category "${category}"` : "";
        const patternSummary = predictions.length === 0
...
exports.DeadlinePredictionAgent = DeadlinePredictionAgent;
// --- pattern detection --------------------------------------------------------
function parseDate(dateStr) {
    const d = new Date(dateStr);
    return isNaN(d.getTime()) ? null : d;
}
function groupByMonthDay(dates) {
 ...
 */
function detectQuarterlyInterval(dates) {
    if (dates.length < 4)
        return null;
    const intervals = [];
    ...
    for (const iv of intervals) {
        if (Math.abs(iv - avg) > 30)
            return null;
    }
    return avg;
}
function detectPatterns(deadlineStrings, nowMs, cutoffMs) {

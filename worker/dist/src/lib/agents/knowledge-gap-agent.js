...
exports.KnowledgeGapAgent = void 0;
...
class KnowledgeGapAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async run(triggerSource) {
        ...
        try {
            ...
            const present = new Set((kbRows ?? []).map((r) => r.category));
            ...
            const response = await (0, claude_1.callClaude)({
                ...
                prompt: "For each missing KB category, write one specific helpful sentence telling a nonprofit what information to add. Categories: " +
                    missing.join(", "),
            });
            ...
            await this.createNotification("knowledge_gap", "Knowledge Base gaps identified", `${missing.length} standard categor${missing.length === 1 ? "y is" : "ies are"} missing from your Knowledge Base: ${missing.join(", ")}.`, { missingCategories: missing, suggestions: suggestionsByCategory });
            ...
            const message = err instanceof Error ? err.message : "Knowledge gap run failed.";
...
exports.KnowledgeGapAgent = KnowledgeGapAgent;

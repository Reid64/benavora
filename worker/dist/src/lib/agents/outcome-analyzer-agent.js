...
exports.OutcomeAnalyzerAgent = void 0;
...
class OutcomeAnalyzerAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async run(triggerSource) {
        ...
        try {
            const { data: outcomeRows, error: outcomesError } = await this.supabase
                .from("outcomes")
                .select("id, result, awarded_amount, requested_amount, opportunity_category, funder_category")
            ...
            const successRateByCategory = {};
            for (const [category, tally] of tallies) {
                if (tally.total >= MIN_OUTCOMES_FOR_RATE) {
                    successRateByCategory[category] = Number((tally.awarded / tally.total).toFixed(3));
                }
            }
            // averageAwardSize.
            const awardedOutcomes = outcomes.filter((o) => o.result === "awarded" && o.awarded_amount != null);
            const averageAwardSize = awardedOutcomes.length > 0
                ? Math.round(awardedOutcomes.reduce((sum, o) => sum + (o.awarded_amount ?? 0), 0) / awardedOutcomes.length)
                : 0;
            // dollarEfficiency: total dollars awarded / total dollars requested.
            const totalRequested = outcomes.reduce((sum, o) => sum + (o.requested_amount ?? 0), 0);
            const totalAwarded = outcomes.reduce((sum, o) => sum + (o.awarded_amount ?? 0), 0);
            const dollarEfficiency = totalRequested > 0
            ...
            const insightResponse = await (0, claude_1.callClaude)({
                maxTokens: 200,
                prompt: `In 2 sentences, summarize the funding performance of this org based on: ${JSON.stringify(stats)}`,
            });
            ...
            const { data: orgRow } = await this.supabase
                ...
                .eq("id", this.orgId)
                .maybeSingle();
            const existingAnalytics = orgRow?.analytics ?? {};
            const { error: updateError } = await this.supabase
                ...
                .eq("id", this.orgId);
            if (updateError) {
                throw new Error(`Failed to persist organization analytics: ${updateError.message}`);
            }
            ...
            const message = err instanceof Error ? err.message : "Outcome analysis run failed.";
...
exports.OutcomeAnalyzerAgent = OutcomeAnalyzerAgent;

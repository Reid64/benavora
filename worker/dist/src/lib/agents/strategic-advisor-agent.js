...
exports.StrategicAdvisorAgent = void 0;
...
const URGENCY_LEVELS = ["immediate", "urgent", "normal", "low"];
function isRecommendationCategory(value) {
    return (typeof value === "string" &&
        RECOMMENDATION_CATEGORIES.includes(value));
}
...
function validateRecommendation(raw) {
    if (!isRecommendationCategory(raw.recommendation_category))
        return null;
    ...
    const confidence_score = Number.isFinite(confidenceRaw)
        ...
        : 60;
    const data_basis = raw.data_basis && typeof raw.data_basis === "object"
        ? raw.data_basis
        : {};
    return {
        ...
        urgency,
        time_sensitivity: typeof raw.time_sensitivity === "string" ? raw.time_sensitivity : null,
        expected_impact: typeof raw.expected_impact === "string" ? raw.expected_impact : null,
        confidence_score,
        data_basis,
    };
}
class StrategicAdvisorAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async loadSuccessRateSnapshot() {
        ...
        for (const [category, tally] of tallies) {
            if (tally.total >= constants_1.MIN_OUTCOMES_FOR_RATE) {
                byCategory[category] = Number((tally.awarded / tally.total).toFixed(3));
            }
        }
        return {
            overallRate: rows.length >= constants_1.MIN_OUTCOMES_FOR_RATE
    ...
    async loadLatestForecast() {
        const { data } = await this.supabase
            ...
            .select("forecast_period, projected_min, projected_max, projected_most_likely, confidence, key_risks, key_opportunities, recommended_actions")
     ...
     * run. */
    async loadOptionalOrgRows(table, columns) {
        try {
            const { data, error } = await this.supabase
                .from(table)
                .select(columns)
    ...
    async loadPendingRelationshipRecommendations() {
        const { data } = await this.supabase
    ...
    async loadUnreadCriticalReputationAlerts() {
        const { data } = await this.supabase
        ...
        return rows
            ...
            .filter((s) => s !== null && (s.severity === "critical" || s.severity === "high"));
    }
    ...
    async loadDominantFunderCategory() {
        const { data } = await this.supabase
            .from("outcomes")
    ...
    async loadLearningPatterns(funderCategory) {
    ...
    buildPrompt(context) {
        const system = "You are a strategic advisor to a nonprofit executive director. " +
        ...
        const lines = [
            ...
            context.successRate.overallRate != null
                ...
                : `12-month success rate: insufficient data (n=${context.successRate.sampleSize}, ` +
                    `need ${constants_1.MIN_OUTCOMES_FOR_RATE}+)`,
            `Success rate by category: ${JSON.stringify(context.successRate.byCategory)}`,
        ];
        if (context.forecast) {
            lines.push(`Latest funding forecast (${context.forecast.forecast_period}): ` +
        ...
        lines.push(`Pending relationship recommendations (${context.relationshipRecs.length}): ` +
            (context.relationshipRecs
                .map((r) => `[${r.urgency}] ${r.entity_type}: ${r.recommendation_text}`)
                .join(" | ") || "none"));
        lines.push(`Unread critical/high reputation alerts (${context.reputationAlerts.length}): ` +
            (context.reputationAlerts
                .map((r) => `[${r.severity}] ${r.entity_type}: ${r.headline}`)
                .join(" | ") || "none"));
        lines.push(`Deadlines due in the next 90 days (${context.upcomingDeadlines.length}): ` +
            (context.upcomingDeadlines
                .map((d) => `${d.due_date} - ${d.title} (${d.deadline_type})`)
                .join(" | ") || "none"));
        lines.push(`Cross-org platform learning patterns for this org's dominant funder ` +
            `category (${context.learningPatterns.length}): ` +
            (context.learningPatterns
                .map((p) => `[${p.pattern_type}] ${p.pattern_content} (sample_count=${p.sample_count}${p.success_rate != null ? `, success_rate=${p.success_rate}` : ""})`)
                .join(" | ") || "none available yet"));
        lines.push(`ROI insights (${context.roiInsights.length}), community need signals ` +
        ...
        lines.push("", "Lead with the most actionable item. Prioritize by urgency and by how", "many independent signals above point at the same conclusion. Do not", "invent numbers the data above doesn't support.");
    ...
    async run(triggerSource) {
        ...
        const decisions = [];
        try {
            const [pipeline, successRate, forecast, roiInsights, communityNeedSignals, corporateIntentSignals, relationshipRecs, reputationAlerts, upcomingDeadlines,] = await Promise.all([
            ...
            for (const rec of validated) {
                ...
                if (insertError || !inserted) {
                    errors.push(`Failed to insert recommendation "${rec.title}": ${insertError?.message ?? "no row returned"}`);
                    continue;
                }
                const recommendationId = inserted.id;
                if (rec.urgency === "immediate")
                    immediateCount += 1;
                else if (rec.urgency === "urgent")
                    urgentCount += 1;
                ...
                if (rec.urgency === "immediate") {
                    await this.createNotification("strategic_recommendation", rec.title, rec.recommendation, {
                        recommendationId,
            ...
            const recommendationsGenerated = immediateCount + urgentCount + normalCount + lowCount;
            const summary = `Generated ${recommendationsGenerated} recommendation(s): ` +
            ...
            const message = err instanceof Error ? err.message : "Strategic advisor run failed.";
...
exports.StrategicAdvisorAgent = StrategicAdvisorAgent;

...
exports.AutonomousDigestAgent = void 0;
const autonomous_base_1 = require("@/lib/agents/autonomous-base");
const claude_1 = require("@/lib/ai/claude");
class AutonomousDigestAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async run(triggerSource) {
        ...
        try {
            ...
            const [discoveredRes, scoredRes, highScoreRes, draftsGeneratedRes, draftsPendingReviewRes, reputationAlertsRes, relationshipRecsRes,] = await Promise.all([
                this.supabase
            ...
            const totalActivity = discovered +
                scored +
                highScore +
                draftsGenerated +
                draftsPendingReview +
            ...
            const userPrompt = `Write a 3-sentence morning briefing. Overnight your AI platform discovered ${discovered} new opportunities, ` +
            ...
            const response = await (0, claude_1.callClaude)({
                system: "You are writing a concise morning briefing for a nonprofit executive director.",
                prompt: userPrompt,
                model: claude_1.DEFAULT_MODEL,
                maxTokens: 400,
            });
            await this.createNotification("morning_digest", "Good morning -- here is what happened overnight", response.text, {
                summary: {
            ...
            const message = err instanceof Error ? err.message : "Morning digest failed.";
...
exports.AutonomousDigestAgent = AutonomousDigestAgent;

...
exports.RenewalTrackerAgent = void 0;
...
class RenewalTrackerAgent extends autonomous_base_1.AutonomousAgent {
    ...
    async run(triggerSource) {
        ...
        try {
            const { data: awardedRows, error: awardedError } = await this.supabase
                ...
                .eq("stage", "awarded");
            if (awardedError) {
                throw new Error(`Failed to load awarded applications: ${awardedError.message}`);
            }
            const awardedApps = (awardedRows ?? []);
            const opportunityIds = Array.from(new Set(awardedApps.map((a) => a.opportunity_id)));
            ...
            const recurringOpps = (oppRows ?? []).filter((o) => o.funder_id && isRecurringOpportunity(o.recurrence));
            for (const opp of recurringOpps) {
                ...
                try {
                    ...
                    const { data: existing } = await this.supabase
                        ...
                        .maybeSingle();
                    if (existing)
                        continue; // renewal already tracked for this cycle.
                    ...
                    if (insertError || !inserted) {
                        errors.push(`opportunity ${opp.id}: failed to create renewal: ${insertError?.message ?? "no row returned"}`);
                        continue;
                    }
            ...
            const message = err instanceof Error ? err.message : "Renewal tracker run failed.";
...
exports.RenewalTrackerAgent = RenewalTrackerAgent;

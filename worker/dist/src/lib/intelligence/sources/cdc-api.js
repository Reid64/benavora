...
async function fetchSocrata(url, params) {
    ...
    if (!res.ok)
        throw new Error(`CDC Socrata error: ${res.status} ${res.statusText}`);
...
class CdcDataSource {
    ...
    async fetchMortalityData(state) {
        const fullState = toFullStateName(state);
        try {
            const rows = await fetchSocrata('https://data.cdc.gov/resource/bi63-dtpu.json', { state: fullState, $order: 'year DESC' });
            if (rows.length === 0)
                return [];
            // Find the most recent year available
            const latestYear = Math.max(...rows
    ...
    async fetchSubstanceAbuseData(state) {
        const fullState = toFullStateName(state);
        const points = [];
        // 1. CDC drug overdose deaths (provisional, 12-month rolling)
        try {
            const overdoseRows = await fetchSocrata('https://data.cdc.gov/resource/xkb8-kh2a.json', {
                state: fullState,
        ...
        try {
            const brfssRows = await fetchSocrata('https://data.cdc.gov/resource/dttw-5yxu.json', {
...
exports.CdcDataSource = CdcDataSource;

...
function normalizeState(state) {
    ...
    if (upper.length === 2 && upper in STATE_ENTITY_IDS)
        return upper;
...
class HudDataSource {
    async fetchHomelessCounts(state, year = 2023) {
        ...
        try {
            ...
            if (!pageRes.ok)
                throw new Error(`HUD Exchange error: ${pageRes.status}`);
            ...
            if (!res.ok)
                throw new Error(`HUD Exchange CSV error: ${res.status}`);
            ...
            for (const line of lines.slice(1)) {
                ...
                if (stateIdx >= 0 && cols[stateIdx]?.toUpperCase() !== abbr)
                    continue;
                if (yearIdx >= 0 && cols[yearIdx] !== String(year))
                    continue;
    ...
    async fetchFairMarketRents(state, county) {
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(`HUD FMR error: ${res.status}`);
    ...
    async fetchAffordabilityIndex(state) {
        const apiKey = process.env.HUD_API_KEY;
        if (!apiKey)
            return [];
        const abbr = normalizeState(state);
        ...
        try {
            ...
            if (!res.ok)
                throw new Error(`HUD CHAS error: ${res.status}`);
            const json = (await res.json());
...
exports.HudDataSource = HudDataSource;

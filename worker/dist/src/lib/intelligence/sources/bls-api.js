...
async function fetchBlsSeries(seriesIds, startYear = '2022', endYear = '2023') {
...
function getMostRecentAnnual(data) {
    const annual = data.filter((d) => d.period === 'M13');
    if (annual.length > 0)
        return annual.sort((a, b) => parseInt(b.year) - parseInt(a.year))[0];
...
class BlsDataSource {
    ...
    async fetchUnemploymentRate(state, county) {
    ...
    async fetchWageData(state, occupation) {
        const stateFips = getStateFips(state);
        if (!stateFips)
            return [];
...
exports.BlsDataSource = BlsDataSource;

...
exports.CensusDataSource = void 0;
...
async function getCountyFips(stateFips, county) {
    // Already a raw 3-digit FIPS code
    if (/^\d{3}$/.test(county.trim()))
        return county.trim();
...
async function fetchCensus(variables, stateFips, countyFips) {
...
class CensusDataSource {
    async fetchPovertyData(state, county) {
        ...
        return rows.flatMap((row) => {
            ...
            if (isNaN(total) || isNaN(belowPoverty))
                return [];
            return [
                {
    ...
    async fetchHousingData(state, county) {
        const stateFips = getStateFips(state);
        if (!stateFips)
            return [];
        const countyFips = county ? await getCountyFips(stateFips, county) : undefined;
    ...
    async fetchDemographics(state, county) {
        const stateFips = getStateFips(state);
        if (!stateFips)
            return [];
        const countyFips = county ? await getCountyFips(stateFips, county) : undefined;
...
exports.CensusDataSource = CensusDataSource;

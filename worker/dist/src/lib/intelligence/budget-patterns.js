...
class BudgetPatternLibrary {
    async getTemplateByCategory(programCategory, grantType) {
    ...
    getFederalCostPrinciples(costCategory) {
    ...
    getTypicalFringeRate(orgType, state) {
    ...
    getIndirectCostRateGuidance(grantType) {
        const type = grantType.toLowerCase();
        if (type === 'federal') {
            return ('Federal grants: Organizations without a negotiated indirect cost rate agreement (NICRA) may use the 10% de minimis rate on Modified Total Direct Costs (MTDC) per 2 CFR 200.414. ' +
        ...
        if (type === 'state') {
            return ('State grants: Indirect cost treatment varies by state agency. Most state agencies accept either the organization\'s federally negotiated rate or the federal 10% de minimis rate. ' +
        ...
        if (type === 'foundation' || type === 'private_foundation' || type === 'corporate') {
            return ('Foundation/corporate grants: Indirect costs are frequently disallowed or capped (typically 10–15% of direct costs). ' +
        ...
        return ('Indirect cost treatment depends on funder type. Federal: de minimis 10% MTDC or negotiated rate. ' +
    ...
    async generateBudgetNarrative(budget, grantType, orgProfile) {
...
exports.BudgetPatternLibrary = BudgetPatternLibrary;

...
class WebhookNotifier {
    async notify(params) {
        ...
        const matching = (configs ?? []).filter((c) => Array.isArray(c.events) && c.events.includes(event));
        if (matching.length === 0)
            return;
        await Promise.allSettled(matching.map(async (config) => {
            try {
                const payload = config.type === 'slack'
                ...
                console.warn(`[WebhookNotifier] Failed to POST to ${config.webhook_url}:`, err instanceof Error ? err.message : String(err));
            }
        }));
    }
}

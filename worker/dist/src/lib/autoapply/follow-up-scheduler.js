...
exports.sendSingleFollowUp = sendSingleFollowUp;
...
async function scheduleFollowUps(params) {
...
async function generateFollowUpContent(templateType, funderName, funderEmail, submittedAt, requestType, orgName) {
    ...
    const prompt = promptMap[templateType] ??
 ...
 */
async function sendSingleFollowUp(item, supabase) {
    try {
        ...
        const { data: funder } = await supabase
            ...
            .maybeSingle();
        const funderName = funder?.name ?? "Valued Funder";
        ...
        const { data: contact } = await supabase
            ...
            .limit(1)
            .maybeSingle();
        const funderEmail = contact?.email ?? null;
        if (!funderEmail) {
        ...
        const replyTo = org?.email ?? "noreply@benavora.com";
        // Generate follow-up email content.
        const content = await generateFollowUpContent(item.template_type, funderName, funderEmail, submission?.submitted_at ?? null, submission?.request_type ?? null, orgName);
...
async function cancelFollowUps(submissionId, reason, supabase) {

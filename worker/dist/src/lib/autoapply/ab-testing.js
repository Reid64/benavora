...
class ABTestEngine {
     ...
     */
    async getVariant(orgId, funderCategory, supabase) {
        try {
     ...
     */
    async recordOutcome(variantId, success, supabase) {
        try {
            const { data } = (await supabase
                .from('ab_test_variants')
                .select('*')
            ...
            if (newCount >= 50) {
                await this.checkForWinner(data.organization_id, data.funder_category, supabase);
            }
        }
        catch {
     ...
     */
    async checkForWinner(orgId, funderCategory, supabase) {
        try {
            const { data } = (await supabase
                .from('ab_test_variants')
                .select('*')
            ...
            if (data.some((v) => v.submission_count < 50))
                return;
            ...
            const rateBest = best.submission_count > 0 ? best.success_count / best.submission_count : 0;
            const rateSecond = second.submission_count > 0 ? second.success_count / second.submission_count : 0;
            // Require >20% relative lift AND 95% statistical confidence.
            const hasLift = rateSecond === 0 || rateBest > rateSecond * 1.2;
            const z = zScore(best.submission_count, best.success_count, second.submission_count, second.success_count);
            const isSig = z > 1.96;
            if (hasLift && isSig) {
                await supabase
                    .from('ab_test_variants')
                    ...
                    .eq('id', best.id);
                console.log(`[ABTestEngine] A/B test winner for ${funderCategory}: ${best.variant_name} at ${(rateBest * 100).toFixed(1)}%`);
            }
        }
        catch {
            // Non-fatal.
     ...
     */
    async getTestResults(orgId, supabase) {
        try {
            const { data } = (await supabase
                .from('ab_test_variants')
                .select('*')
            ...
            return Array.from(byCategory.entries()).map(([category, vars]) => ({
                ...
                variants: vars.map((v) => ({
                    ...
                    conversionRate: v.submission_count > 0 ? v.success_count / v.submission_count : 0,

...
function getEncryptionKey() {
    ...
    if (!key)
        throw new Error("CREDENTIAL_ENCRYPTION_KEY env var is not set");
    const derived = crypto_1.default.scryptSync(key, "benavora-cred-salt", 32);
    return derived;
}
...
class CredentialManager {
    ...
    async getCredentials(organizationId, funderId) {
    ...
    async updateLastLogin(credentialId, success) {
        const { error } = await this.supabase
            .from("funder_credentials")
            .update({

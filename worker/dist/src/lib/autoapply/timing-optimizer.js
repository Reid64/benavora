...
exports.getTimingScore = getTimingScore;
exports.shouldDelaySubmission = shouldDelaySubmission;
...
function normalizeFunderType(funderType, funderCategory) {
    const lower = funderType.toLowerCase();
    if (lower === 'government' || lower === 'gov')
        return 'government';
    if (lower === 'foundation')
        return 'foundation';
    if (lower === 'community')
 ...
 */
function shouldDelaySubmission(score, threshold = 0.6, optimalMonths) {
    if (score >= threshold)
        return { delay: false };

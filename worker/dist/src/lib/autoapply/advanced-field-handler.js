...
exports.AdvancedFieldHandler = void 0;
class AdvancedFieldHandler {
    async fillSelect(page, selector, value) {
        try {
            ...
            const exact = options.find((o) => o.value === value || o.text.toLowerCase() === value.toLowerCase());
            if (exact) {
    ...
    async fillCheckbox(page, selector, shouldCheck) {
        const isChecked = await page.isChecked(selector).catch(() => false);
        if (isChecked !== shouldCheck) {
            await page.click(selector);
        }
    }
    async fillRadio(page, name, value) {
    ...
    async fillDatePicker(page, selector, date) {
        try {
            await page.fill(selector, date);
            await page.keyboard.press('Tab');
            const confirmedValue = await page.$eval(selector, (el) => el.value).catch(() => '');
            if (confirmedValue && confirmedValue !== '')
                return true;
            await page.click(selector);
            ...
            const dayEl = await page.$$eval(daySelector, (els, d) => {
    ...
    async fillFileUpload(page, selector, filePath) {
        try {
    ...
    async handleConditionalFields(page) {
...
exports.AdvancedFieldHandler = AdvancedFieldHandler;

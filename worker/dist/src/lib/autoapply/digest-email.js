...
exports.sendAutoapplyDigest = sendAutoapplyDigest;
...
function failureAction(errorMessage) {
    if (!errorMessage)
...
async function sendAutoapplyDigest({ organizationId, supabase, since, }) {
    ...
    const failed = rows.filter((r) => ["failed", "captcha_blocked", "account_required", "site_error"].includes(r.status));
    ...
    const { data: admins } = await supabase
        ...
        .in("role", ["owner", "admin"])
        .limit(5);
    const emailList = (admins ?? []).map((p) => p.email).filter(Boolean);

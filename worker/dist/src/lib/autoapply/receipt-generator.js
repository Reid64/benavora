...
const LH = 15;
const C_DARK_BLUE = (0, pdf_lib_1.rgb)(0.05, 0.1, 0.35);
const C_WHITE = (0, pdf_lib_1.rgb)(1, 1, 1);
const C_BLACK = (0, pdf_lib_1.rgb)(0, 0, 0);
const C_GRAY = (0, pdf_lib_1.rgb)(0.4, 0.4, 0.4);
const C_LIGHT_GRAY = (0, pdf_lib_1.rgb)(0.75, 0.75, 0.75);
const C_DARK_GRAY = (0, pdf_lib_1.rgb)(0.2, 0.2, 0.2);
const C_GREEN = (0, pdf_lib_1.rgb)(0.08, 0.45, 0.18);
const C_HEADER_SUB = (0, pdf_lib_1.rgb)(0.75, 0.75, 0.9);
...
async function generateReceipt(params) {
    const { supabase, submission, funderName, orgName, requestProfile, confirmationData, screenshots, } = params;
    const pdfDoc = await pdf_lib_1.PDFDocument.create();
    const page = pdfDoc.addPage([PAGE_W, PAGE_H]);
    const font = await pdfDoc.embedFont(pdf_lib_1.StandardFonts.Helvetica);
    const bold = await pdfDoc.embedFont(pdf_lib_1.StandardFonts.HelveticaBold);
    let y = PAGE_H - MARGIN;
    const aboveFooter = () => y > 65;
    function drawHRule() {
        page.drawLine({
            start: { x: MARGIN, y },
    ...
    function drawText(content, opts = {}) {
    ...
    if (aboveFooter())
        field("Submission Date", submittedAt);
    if (aboveFooter())
        field("Funder", funderName);
    if (aboveFooter())
        field("Status", "SUBMITTED ✓", C_GREEN);
    ...
    // ── Request Details ──────────────────────────────────────────────────────────
    const reqType = requestProfile?.request_type ?? submission.request_type ?? null;
    const reqAmount = submission.request_amount ?? requestProfile?.min_value ?? null;
    if (aboveFooter() && (requestProfile ?? reqType ?? reqAmount != null)) {
        ...
        if (aboveFooter() && reqAmount != null) {
            ...
            if (maxVal != null && maxVal !== reqAmount) {
                field("Amount Range", `${formatCurrency(reqAmount)} – ${formatCurrency(maxVal)}`);
            }
            else {
    ...
    if (aboveFooter() && confirmationData) {
        ...
        const hasContent = cd.confirmation_number ??
    ...
    const pitch = submission.personalized_pitch ??
    ...
    page.drawText(`Generated by Benavora AutoApply on ${new Date().toLocaleString("en-US", { dateStyle: "long", timeStyle: "short" })}`, { x: MARGIN, y: 36, size: 7, font, color: C_GRAY });
    // ── Serialize PDF ────────────────────────────────────────────────────────────
    ...
    if (uploadError) {
        console.warn("[ReceiptGenerator] Storage upload failed:", uploadError.message);
    }

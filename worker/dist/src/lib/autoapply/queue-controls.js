"use strict";
/* eslint-disable @typescript-eslint/no-explicit-any */
Object.defineProperty(exports, "__esModule", { value: true });
exports.QueueControlPlane = void 0;
class QueueControlPlane {
    async pauseTenant(orgId, reason, pausedBy, supabase) {
        await this.setPaused('tenant', orgId, true, reason, pausedBy, supabase);
    }
    async resumeTenant(orgId, supabase) {
        await supabase
            .from('queue_controls')
            .delete()
            .eq('control_type', 'tenant')
            .eq('target_id', orgId);
    }
    async pauseFunder(funderId, reason, supabase, pausedBy) {
        await this.setPaused('funder', funderId, true, reason, pausedBy ?? null, supabase);
    }
    async resumeFunder(funderId, supabase) {
        await supabase
            .from('queue_controls')
            .delete()
            .eq('control_type', 'funder')
            .eq('target_id', funderId);
    }
    async pauseDomain(domain, reason, supabase, pausedBy) {
        await this.setPaused('domain', domain, true, reason, pausedBy ?? null, supabase);
    }
    async resumeDomain(domain, supabase) {
        await supabase
            .from('queue_controls')
            .delete()
            .eq('control_type', 'domain')
            .eq('target_id', domain);
    }
    async pausePlatform(reason, pausedBy, supabase) {
        console.error(`[QueueControlPlane] PLATFORM PAUSED by ${pausedBy}: ${reason}`);
        await this.setPaused('platform', 'global', true, reason, pausedBy, supabase);
    }
    async resumePlatform(supabase) {
        await supabase
            .from('queue_controls')
            .delete()
            .eq('control_type', 'platform')
            .eq('target_id', 'global');
    }
    /**
     * Check all control levels in descending precedence: platform → domain → funder → tenant.
     * Returns the first block found, or { blocked: false } if clear.
     */
    async isBlocked(params) {
        const { orgId, funderId, portalUrl, supabase } = params;
        // 1. Platform-level kill switch
        const { data: platformRows } = await supabase
            .from('queue_controls')
            .select('reason')
            .eq('control_type', 'platform')
            .eq('target_id', 'global')
            .eq('paused', true)
            .limit(1);
        const platformRow = (platformRows ?? [])[0];
        if (platformRow !== undefined) {
            return {
                blocked: true,
                reason: platformRow.reason ?? 'Platform is paused (emergency kill switch).',
                controlType: 'platform',
            };
        }
        // 2. Domain-level pause
        if (portalUrl) {
            const domain = extractDomain(portalUrl);
            const { data: domainRows } = await supabase
                .from('queue_controls')
                .select('reason')
                .eq('control_type', 'domain')
                .eq('target_id', domain)
                .eq('paused', true)
                .limit(1);
            const domainRow = (domainRows ?? [])[0];
            if (domainRow !== undefined) {
                return {
                    blocked: true,
                    reason: domainRow.reason ?? `Domain "${domain}" is paused.`,
                    controlType: 'domain',
                };
            }
        }
        // 3. Funder-level pause
        if (funderId) {
            const { data: funderRows } = await supabase
                .from('queue_controls')
                .select('reason')
                .eq('control_type', 'funder')
                .eq('target_id', funderId)
                .eq('paused', true)
                .limit(1);
            const funderRow = (funderRows ?? [])[0];
            if (funderRow !== undefined) {
                return {
                    blocked: true,
                    reason: funderRow.reason ?? 'Funder submissions are paused.',
                    controlType: 'funder',
                };
            }
        }
        // 4. Tenant (org) level pause
        if (orgId) {
            const { data: tenantRows } = await supabase
                .from('queue_controls')
                .select('reason')
                .eq('control_type', 'tenant')
                .eq('target_id', orgId)
                .eq('paused', true)
                .limit(1);
            const tenantRow = (tenantRows ?? [])[0];
            if (tenantRow !== undefined) {
                return {
                    blocked: true,
                    reason: tenantRow.reason ?? 'Organization submissions are paused.',
                    controlType: 'tenant',
                };
            }
        }
        return { blocked: false };
    }
    /** Return all currently active (paused=true) controls. */
    async getStatus(supabase) {
        const { data } = await supabase
            .from('queue_controls')
            .select('*')
            .eq('paused', true)
            .order('created_at', { ascending: false });
        return (data ?? []);
    }
    // Delete any existing rows for this control_type+target_id, then insert a fresh one.
    // Avoids the need for a UNIQUE constraint on (control_type, target_id).
    async setPaused(controlType, targetId, paused, reason, pausedBy, supabase) {
        await supabase
            .from('queue_controls')
            .delete()
            .eq('control_type', controlType)
            .eq('target_id', targetId);
        await supabase.from('queue_controls').insert({
            control_type: controlType,
            target_id: targetId,
            paused,
            paused_by: pausedBy,
            paused_at: new Date().toISOString(),
            reason,
        });
    }
}
exports.QueueControlPlane = QueueControlPlane;
function extractDomain(url) {
    try {
        return new URL(url).hostname.replace(/^www\./, '');
    }
    catch {
        return url;
    }
}

...
function findSubpageLinks(homepageHtml, baseUrl) {
    const $ = cheerio.load(homepageHtml);
    const origin = new URL(baseUrl).origin;
    const found = new Map();
    const seenUrls = new Set([baseUrl]);
    $("a[href]").each((_i, el) => {
        if (found.size >= SUBPAGE_CATEGORIES.length)
            return;
        ...
        for (const category of SUBPAGE_CATEGORIES) {
            if (found.has(category.key))
                continue;
            if (!category.regex.test(haystack))
                continue;
            let absoluteUrl;
            try {
            ...
            if (!absoluteUrl.startsWith(origin) || seenUrls.has(absoluteUrl))
                continue;
...
function buildFoundationPrompt(siteText) {
...
function buildDonorProspectPrompt(siteText) {
...
function asKeyPeople(v) {
    ...
    for (const item of v) {
        ...
        if (!name)
            continue;
        out.push({ name, title: asStringOrNull(item.title) ?? "" });
    }
    return out;
}
...
function normalizeFoundationExtraction(value) {
    return {
...
function normalizeDonorProspectExtraction(value) {
    return {
...
async function extractFromWebsite(url, schema) {
    try {
        const homepageUrl = normalizeUrl(url);
        if (!homepageUrl) {
            return { ok: false, data: null, error: `Invalid URL: "${url}"` };
        }
        const homepage = await (0, crawler_core_1.fetchCompliant)(homepageUrl, { adapterName: ADAPTER_NAME });
        ...
        const pages = [{ url: homepageUrl, html: homepage.html }];
        const subpageUrls = findSubpageLinks(homepage.html, homepageUrl);
        for (const subpageUrl of subpageUrls) {
            const result = await (0, crawler_core_1.fetchCompliant)(subpageUrl, { adapterName: ADAPTER_NAME });

...
const samRateLimiter = new crawler_core_1.DomainRateLimiter(Math.ceil(60_000 / 450));
...
exports.SamGovError = SamGovError;
function getApiKey() {
...
function normalizeEntity(raw, naicsCode) {
    ...
    return {
        ...
        geo: null, // Entity API doesn't return lat/lng.
        phone: null,
 ...
 */
async function searchEntitiesByNaics(naicsCode, opts = {}) {
...
function formatAwardeeAddress(location) {
    if (!location)
        return null;
 ...
 */
async function searchRecentAwardRecipients(daysBack = 90) {
    const apiKey = getApiKey();
    await samRateLimiter.acquire(SAM_HOST);

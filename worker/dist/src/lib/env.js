...
exports.warnIfMissingOptionalEnv = warnIfMissingOptionalEnv;
 ...
 */
function warnIfMissingOptionalEnv(key) {
    ...
    if (process.env[key])
        return;
    warnedKeys.add(key);
    console.warn(`[env] Optional env var ${key} is not set — ${entry.usedBy} will be unavailable.`);
}

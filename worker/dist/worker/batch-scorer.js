...
exports.scoreAndReorderQueue = scoreAndReorderQueue;
const timing_optimizer_js_1 = require("../src/lib/autoapply/timing-optimizer.js");
async function scoreAndReorderQueue(supabase, organizationId) {
    ...
    const { data: items, error: queueError } = await supabase
        ...
        .eq('status', 'pending');
    if (queueError || !items || items.length < 5) {
        return { reordered: 0 };
    }
    const queueRows = items;
    const funderIds = [
        ...new Set(queueRows.map((r) => r.funder_id).filter((id) => id !== null)),
    ];
    ...
    const { data: fundersData } = await supabase
        ...
        .in('id', funderIds);
    const funderMap = new Map((fundersData ?? []).map((f) => [f.id, f]));
    ...
    const { data: templatesData } = await supabase
        ...
        .select('funder_id')
        .in('funder_id', funderIds);
    const templateFunderIds = new Set((templatesData ?? [])
        .map((t) => t.funder_id)
        .filter((id) => id !== null));
    ...
    const { data: givingData } = await supabase
        ...
        .eq('organization_id', organizationId)
        .in('funder_id', funderIds);
    const givingFunderIds = new Set((givingData ?? [])
        .map((r) => r.funder_id)
        .filter((id) => id !== null));

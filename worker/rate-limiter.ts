...
export class RateLimiter {
  async waitBetweenSubmissions(): Promise<void> {
  ...
  async canSubmitToDomain(funderId: string): Promise<boolean> {
    ...
    if (data !== null) {
      const lastSubmitted = data.submitted_at as string;
  ...
  getBackoffDelay(errorType: string, retryCount: number): number {
    switch (errorType) {
      case 'site_error':
      case 'timeout': {

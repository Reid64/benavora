...
## "Explore the Platform First" Onboarding Bypass — August 20, 2026 (WGR-154)

Investigated a report that clicking "Explore the platform first" on `/onboarding` produced dev-server activity but no visible navigation. Live-tested the existing mechanism (`src/app/(dashboard)/onboarding/page.tsx`'s `handleExploreFirst` sets a `benavora_onboarding_skip` cookie; `src/middleware.ts`'s onboarding gate checks it) 5 times against current `main` — all 5 succeeded, matching `INTERACTION_MAPS_v2.md`'s documented design exactly. Could not force a hard reproduction of the reported symptom.

Found and fixed two real defects in the same code path regardless (both the WGR-111 over-broad-middleware pattern): (1) the onboarding gate in `src/middleware.ts` applied to `/api/*` routes, silently 307-redirecting background polling (`/api/nav-counts`, `/api/notifications`) made while on `/onboarding` to the onboarding HTML page instead of JSON — confirmed on every test run, error swallowed client-side; fixed by excluding `/api/*` from the gate. (2) the click handler used Next's client-side router (`router.push`), which can serve a stale prefetched response for a destination whose access conditions changed — the likely explanation for an intermittent real-world failure a fixed-timing script wouldn't trigger; fixed by switching to a hard `window.location.href` navigation.

Live-re-verified: nav-counts polling no longer redirected while on `/onboarding` (0/6 vs. 100% before); clicking the button lands on `/dashboard`; onboarding progress stays intact and resumable ("1 of 7 · Resume setup" banner still shows — bypass, not deletion). 7/7 automated assertions pass. Evidence in `test-evidence/remediation/onboarding-bypass/`. Register entry: `test-evidence/_register/WIRING_GAP_REGISTER.md` WGR-154 (`RESOLVED`). `pnpm run build` exit 0.

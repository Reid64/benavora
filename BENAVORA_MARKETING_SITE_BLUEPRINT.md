...
| Queue | Scope | Gate evidence |
|---|---|---|
| mkt-00 | Theme tokens, marketing layout shell, nav + mega-menu, footer, MDX loader, route skeletons for all 22 pages, redirects from old routes | All 22 routes 200; nav computed styles; contrast ratios from Section 3 |
| mkt-01 | Fixture capture session: IX-01 and IX-06 recorded from prod as Faith Foundation; IX-02/IX-03 fixtures generated and frozen; `public/demo/fixtures.json` | Files present, schema-validated |
| mkt-02 | IX-01 AutoApply Replay component + Home page | Replay loops, scrub works, zero network calls in Playwright HAR |
| mkt-03 | Platform template + 6 platform pages with IX-02, IX-03, IX-06 | 6 routes, each demo renders, HAR shows no app API calls |
| mkt-04 | IX-04 Lifecycle Explorer + /how-it-works; IX-05 Agent Explorer + /agents | Interaction smoke |
| mkt-05 | Solutions template + 6 solutions pages | 6 routes |
| mkt-06 | /why-benavora, /trust, /company, /pricing with IX-07 | 4 routes |
| mkt-07 | Knowledge schema migration, ingestion script, CORPUS_SEED.yaml Tier 1-2, first ingest run | Ingest report, chunk count, RLS test |
| mkt-08 | `/api/public/assist` + IX-08 floating widget on all marketing pages; in-app surface wiring behind existing auth | Public answers cite sources; tenant tool calls org-scoped (vitest) |
| mkt-09 | /resources library shell, /demo booking, sitemap, OG images, analytics events on every CTA | Lighthouse SEO >= 95, sitemap valid |
| mkt-10 | Live smoke on www.benavora.com after `npx vercel deploy --prod`; screenshots of all 22 pages; register rows for any defect | 22/22 PASS evidence |

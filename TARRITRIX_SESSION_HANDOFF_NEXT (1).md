...
This is the actual next real build step: activating the highest-value currently-disabled
write actions in Launch Center, narrow and watched (not a broad unattended sweep — that
pattern caused real problems in the prior session). Confirm the sitemap fix and governance
sync from the prior session actually landed first (`git log --oneline -10` should show
recent commits ending near a governance-sync commit), then run this:

```
cd C:\Users\manag\Documents\Tarritrix

claude --dangerously-skip-permissions "Activate 3 currently-disabled Launch Center write actions, one at a time, each fully verified before moving to the next — page build, map stacking, indexing. Real endpoints already exist for all three (A-02/A-04/A-08 trigger routes), they are just disabled in the UI per Contract 9's default-off policy.

For each of the 3, in order:
1. Read the actual Launch Center component and the actual trigger route it should call — confirm the real request shape (client_id and whatever else the route requires) via direct code read, do not assume it matches the earlier crawl/sitemap fix's shape.
2. Enable the button, wired to the real endpoint with the real selected client's data (use the existing useClientContext() pattern already established this session).
3. Verify live: with a real client selected (E4 Construction & Roofing), click the action, confirm a real 200/success response AND a real downstream effect (a real agent_events row, and for page build specifically, confirm a real page actually gets created — not just that the trigger accepted the request).
4. If the action requires a scope/confirmation step (e.g. which pages, which locations) that doesn't currently exist in the UI, build the minimal real version of that — do not fire an ambiguous bulk action with no scoping.
5. Commit and push after each of the 3 individually.

If any of the 3 turns out to need real backend work beyond wiring (e.g. the trigger route itself has a bug, or a required scoping concept doesn't exist anywhere), stop on that one, report exactly what's missing, and move to the next rather than trying to build new backend inline.

Update STATE_OF_THE_BUILD.md noting these 3 are now live-triggering, not just displayed. Verify production matches HEAD via vercel --prod after the final commit."
```

Watch this run — don't background it via FORGE (see the standing rules below for why).
Once these 3 are confirmed working live by you directly, the rest of the handoff below
gives full context for whatever comes after.

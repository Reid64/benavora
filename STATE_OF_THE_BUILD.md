...
- **#15 (form-filler approval gate)**: adding the required approval check meant `worker/queue-processor.ts`'s existing call to `form-filler-agent.ts`'s `fillAndSubmit()` — which never passed a `sessionId` — would always throw "Submission blocked: session not approved," disabling the worker's autonomous-submission path entirely. **RESOLVED, same day**: the worker now creates and approves a real `automation_sessions` row per submission before calling `fillAndSubmit()`, and finalizes it to `submitted`/`failed` afterward — a genuine audit trail, not a bypass (see #15 below for the full design and a second, related bug it surfaced and avoided repeating).
- **Migration 045's RLS policies were broken in production** (found while fixing #15/building migration 065): `form_templates`, `autoapply_submissions`, and `submission_queue` all had RLS policies referencing an `organization_members` table that doesn't exist in this schema — confirmed live (`relation "public.organization_members" does not exist`). These 3 tables are queried via the session-scoped (RLS-enforced) client in real user-facing routes (`GET/POST /api/autoapply/queue`, `POST /api/autoapply/templates/test`, `GET /api/autoapply/profiles`), so this wasn't dormant — those endpoints have been broken for real users. **Fixed via migration `066_fix_autoapply_rls_policies.sql`**, applied to production the same day: all 12 policies (4 actions × 3 tables) replaced with the correct `profiles.organization_id`-derived pattern (matching migration 020).
...
15. **[LOW] Dual form-filler stacks with different safety behavior — FULLY FIXED 2026-07-03 (gate same day, worker call site same day via a follow-up fix).** `src/lib/autoapply/form-filler-agent.ts`'s `fillAndSubmit()` calls a private `assertSessionApproved()` immediately before `submitForm()`: it requires a `sessionId` and queries `automation_sessions` directly (org-scoped) to confirm `status === 'approved'`, throwing `"Submission blocked: session not approved"` otherwise. Queries the table directly rather than importing `AutomationSessionManager` because this file is inside `worker/tsconfig.json`'s isolated build boundary and cannot import from `src/lib/automation/**`.
    **Follow-up, same day**: `worker/queue-processor.ts` didn't pass a `sessionId` at all, so every submission through this worker would have thrown "Submission blocked: session not approved" — closing the security hole but silently disabling the worker's entire autonomous-submission path. Fixed by adding two private methods, `createApprovedAutomationSession()` and `finalizeAutomationSession()`, called immediately before and after `filler.fillAndSubmit()`. This worker has no per-item human reviewer (that's the point of "AutoApply") — its risk engine already decides upstream whether an item proceeds at all (a `'manual'` recommendation throws `SkipError` and routes to `pending_manual` before ever reaching the fill/submit stage), so a real `automation_sessions` row is created and driven `pending → approved` for anything that reaches this point, recording the risk score/classification/recommendation in `notes` for a genuine audit trail, then finalized to `submitted` or `failed` once the attempt completes. This is not a bypass: the gate function itself is untouched and still requires a real approved row; what changed is that one now always exists by the time it's checked.
    **Second bug found and avoided while fixing this**: `automation_sessions.approved_by` is a `uuid` column, but the *existing*, already-shipped semi-autonomous/autonomous path for grant-application automation (`src/lib/automation/session-manager.ts`'s `markAutoSubmitted()`, called from `src/lib/agents/browser-automation.ts`) writes a non-uuid `system:<automationLevel>` string into that same column — that would fail with "invalid input syntax for type uuid" if it ever actually ran. Not fixed here (separate, already-shipped file, out of scope for this task) but flagged; the new AutoApply worker code deliberately leaves `approved_by` `null` and puts its audit reasoning in `notes` instead, to avoid repeating the same mistake.
...
- **AutoApply: MOSTLY READY.** Core approval-gated browser automation (`StealthBrowser`, `BrowserAutomationAgent`, `AutomationSessionManager`) is real and correctly gates human approval before submission for the primary `api/automation/process` path. Previously listed blockers, now resolved: (a) Follow-Ups is now fully functional — routes plus migration 065 creating the backing table — FIXED #3; (b) the older `form-filler-agent.ts` stack now has an approval gate AND `worker/queue-processor.ts` correctly wires an approved session into it — FIXED #15, fully closed; (d) the form-analysis fallback stub now delegates to real logic — FIXED #13. Item (e), BYO API keys stored plaintext (#6), was already FIXED before this batch. Also fixed this pass: `form_templates`/`autoapply_submissions`/`submission_queue` had broken RLS policies referencing a nonexistent `organization_members` table, breaking `GET/POST /api/autoapply/queue`, `POST /api/autoapply/templates/test`, and `GET /api/autoapply/profiles` for real users — fixed via migration 066. Remaining: (c) `form-analyzer`/`form-filler` API routes still need a Chromium binary unavailable on Vercel and a worker that isn't deployed to Railway (#14, unchanged, informational).
...
### Migrations Applied to Production (054-066)
...
066 `fix_autoapply_rls_policies` applied 2026-07-03 (same day) — replaces all 12 broken `organization_members`-referencing RLS policies on `form_templates`/`autoapply_submissions`/`submission_queue` (from migration 045) with the correct `profiles.organization_id` pattern. No table count change (policy-only fix).

...
**UPDATE 2026-07-03 (same day, second pass): all 25 numbered findings below are now FIXED or explicitly resolved** (21 code fixes in one batch, on top of the 4 security fixes — #1, #5, #6, #8 — already fixed earlier the same day; #14/#24 were INFO-level and required no code change). Each item below is annotated with what changed. Two items surfaced **new, bigger problems than originally scoped** while fixing them, both deliberately *not* silently absorbed:
- **#3 (AutoApply Follow-Ups)**: the routes are now fully code-complete, but the underlying `autoapply_follow_ups` table **does not exist in the production database at all** — confirmed via a live schema query, zero results for the table name or anything matching `%follow%`. `src/lib/autoapply/follow-up-scheduler.ts` (which predates this fix) was therefore *already* completely non-functional in production, not just the frontend routes. No migration was applied without asking first — that's a production DDL change out of scope for "fix the routes," and hard to reverse. **A migration creating this table is required before this feature works at all.**
- **#15 (form-filler approval gate)**: adding the required approval check means `worker/queue-processor.ts`'s existing call to `form-filler-agent.ts`'s `fillAndSubmit()` — which never passed a `sessionId` — will now **always throw "Submission blocked: session not approved"** until that worker call site is updated to supply an approved automation session. This closes the security hole (correct) but disables that code path's autonomous submission entirely until the worker is updated to match.
...
3. **[HIGH] AutoApply Follow-Ups page is fully broken — ROUTES FIXED 2026-07-03, but blocked on a missing table.** Created all 4 missing routes (`GET /api/autoapply/follow-ups`, `GET .../stats`, `PATCH .../[id]`, `PATCH .../cancel-all/[funderId]`), plus extracted `sendSingleFollowUp()` out of `follow-up-scheduler.ts`'s batch loop so the same send logic backs both the scheduled cron path and the page's "Send Now" action. **While building this, discovered the underlying `autoapply_follow_ups` table does not exist in production at all** — confirmed via a live schema query (zero matches for the table name or `%follow%`). This means `follow-up-scheduler.ts` itself has been non-functional in production all along, independent of the frontend routing bug this item originally described. No migration was applied without asking — creating a production table is a DDL change outside "fix the phantom routes," and it's the kind of hard-to-reverse action that warrants a decision from Reid first. **The routes and scheduler are code-complete and ready the moment a migration creates this table; until then, this feature is still non-functional end-to-end.**
...
- **AutoApply: MOSTLY READY, one feature blocked on a missing migration.** Core approval-gated browser automation (`StealthBrowser`, `BrowserAutomationAgent`, `AutomationSessionManager`) is real and correctly gates human approval before submission for the primary `api/automation/process` path. Previously listed blockers, now resolved: (b) the older `form-filler-agent.ts` stack now has an approval gate — FIXED #15 (with the noted worker-side follow-up needed); (d) the form-analysis fallback stub now delegates to real logic — FIXED #13. Item (e), BYO API keys stored plaintext (#6), was already FIXED before this batch. Remaining: (a) Follow-Ups is code-complete but still non-functional — its backing table doesn't exist in production (#3, needs a migration decision); (c) `form-analyzer`/`form-filler` API routes still need a Chromium binary unavailable on Vercel and a worker that isn't deployed to Railway (#14, unchanged, informational).
...
[FIXED, BLOCKED ON MIGRATION] autoapply/follow-ups (845) — all 4 routes now exist — CRITICAL ISSUE #3 (routes FIXED 2026-07-03), but the backing `autoapply_follow_ups` table does not exist in production. Non-functional until a migration creates it.
...
All applied to vbjplpquqxxfbpazyalt on 2026-06-22: 054 email_calendar_integration+funders_contact_email, 055 admin_sales_outreach+sequence_enrollment_variables, 056 four_tier_admin_system, 057 draft_automation_pipeline, 058 backfill_opportunity_deadlines+lead_enrichment_system, 059 budget_patterns, 060 grantmaker_profiles, 061 corporate_giving_targets, 062 community_foundations, 063 white_label, 064 drop orphaned email_threads/email_messages tables. (Confirmed live via this session's schema query: 104 tables total.)

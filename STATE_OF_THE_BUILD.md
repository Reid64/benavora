...
consolidated into two new docs: **`test-evidence/pt-06/PHASE-06-SUMMARY.md`** (every number, cited
to its evidence file) and **`test-evidence/pt-06/REVIEW-PACK.md`** (the short read — start there).
**The headline correction**: the real, settled drift number is **57 of 165 checkable migrations
(34.5%) unapplied**, not the stale "28 of 108" `MIGRATION_AUDIT.md` figure from July — determined
by live-object-existence checking since this project has **no migration-tracking table at all**
(every migration was applied by hand via direct `DATABASE_URL` connection, never via the Supabase
CLI). **The two migration directories' numbering diverged completely from file `072` through `127`**
(56 straight numbers, both trees reusing the same number for unrelated content), plus **8
same-directory duplicate-prefix pairs** (e.g. `052_governance_layer.sql`/`052_webhook_configs.sql`).
**14 of the 57 unapplied files are missing an entire table** — 4 already cause a known production
500 (PT-02's WGR-005/006/008/009); a separate, independent code-vs-schema cross-reference (1,070
files, 11,474 column references) confirms **21 missing tables / 50 column mismatches** referenced by
real, live, non-test code, 53 of 57 unapplied files with a confirmed real call site already waiting
on them. Everything else came back clean or contained: **267 live FKs, zero orphaned rows**; **0
tables without a primary key**; the two `ein`-based unique indexes are genuinely enforced (zero
duplicates at 133,812 and 1,978,526 rows); but **13 of 120 tenant-scoped tables have no live FK back
to `organizations`** (feeds PT-05 directly, with a named table list) and **6 identifier-shaped
columns have no unique index against real duplicate data**. Migration idempotency was actually run
(not left PENDING) against a disposable local database: **6 of 7 sampled files are NOT idempotent**
— 5 fail loudly and safely on re-apply (P2), but **`003_onboarding.sql` silently corrupted a real
seeded row's state on a second run, with zero error** (P1) — a real operational hazard given there's
no tracking table to prevent an accidental double-apply. Register grew from WGR-040 (PT-08's last
row) to **WGR-070** — 30 new rows, the largest single-phase contribution to date, including a fix to
13 pre-existing rows (WGR-041–053) that had an invalid Scope Tag value. **WGR-007's product decision
(apply `followup_sequences` vs. retire for `application_followups`) is still owed — not resolved by
this phase either.** **Recommendation: PT-05 (tenant isolation) next**, using this phase's 13-table
tenant-FK-gap list as the concrete target — not PT-08, which is already complete with its own
summary/review pack recommending PT-09. Nothing in this phase was applied to production; every
finding is diagnosis only, per this audit program's standing read-only rule.

**Prior: August 20, 2026 — audit PT-06-005 COMPLETE: dup/null data-quality on the large tables
...
Consolidation of the five PT-06 sub-audits (connection proof + migration-file inventory, applied-vs-
on-disk drift map, code-vs-live-schema mismatch audit, constraint/FK/orphan integrity, dup/null
data-quality + migration idempotency — each already detailed in its own session entry below) into
`test-evidence/pt-06/PHASE-06-SUMMARY.md` (every number cites its evidence file) and
`test-evidence/pt-06/REVIEW-PACK.md` (the short read for Reid). No new live testing was performed
this session — this was consolidation, register reconciliation, and write-up only, following the
same `PHASE-0X-SUMMARY.md`/`REVIEW-PACK.md` pattern PT-00/PT-02/PT-08 already established.

**Register reconciliation, not just confirmation.** Found the register was NOT already complete for
PT-06: WGR-041 through WGR-062 (migration-drift + schema-mismatch findings from the five sub-audits)
were already present, but the constraint/FK/orphan integrity audit (`integrity.json`), the dup/null
data-quality audit (`data-quality.json`), the migration idempotency test (`idempotency.json`), the
`appliedNotOnDisk` finding (2 live tables with no matching migration file), and the read-only
connection proof (`connection-proof.txt`) had zero register rows referencing them at all — confirmed
by grepping the register for each evidence filename before writing anything. Added **8 new rows,
WGR-063 through WGR-070**, covering all five: FK-orphan check (0 findings, CONFIRMED-OK), tenant FK
gap (13 findings, CONFIRMED-BROKEN, feeds PT-05 directly), unique-index gaps (6 findings), duplicate-
rate findings (3 tables), null-rate findings (3 tables), the migration idempotency test result (RAN,
not PENDING — the `003_onboarding.sql` silent-data-corruption-on-reapply finding is the single worst
finding of this whole phase), the 2 `appliedNotOnDisk` tables (PENDING-SCOPE, needs a human call on
whether to backfill a migration file or document as intentionally hand-managed), and the connection-
proof traceability row. Register grew from 62 to **70 rows total**, continuing numbering cleanly from
PT-08's last row (WGR-040) through PT-06's own WGR-041–070.

**Also fixed a real defect found while reading the register, not part of the original task list**:
rows WGR-041 through WGR-053 (13 rows) had `PT-06` in their Scope Tag cell — not one of the 5 values
the register's own legend defines (`CONFIRMED-BROKEN`/`UNVERIFIED`/`PENDING-SCOPE`/`CONFIRMED-OK`/
`RESOLVED`). Corrected all 13 to `CONFIRMED-BROKEN`, matching every sibling migration-drift row and
the substance of each finding (all are live-verified drift, confirmed via direct `DATABASE_URL`
queries). Verified the fix and the 8 new rows both by re-parsing the markdown table's pipe count
(8 pipes = 7 valid columns) for every touched/added row, not just eyeballing the rendered text.

**Cross-linked the unapplied migrations to PT-02's 5 known 500s, as this task required.** 4 of the 5
(WGR-006/007/008/009) already cited their causing migration file by number in their own row text from
the PT-02 session; confirmed this held rather than assumed. `PHASE-06-SUMMARY.md`'s PT-06-002 table
extends this into a full 14-row table of every missing-whole-table migration found this phase, marking
"None captured" plainly for the 10 that don't correspond to any of PT-00's 5 originally-captured 500s
— not implying those 10 are harmless, only that PT-00's specific smoke-sweep methodology didn't happen
to reach a route surfacing them, while `consumer-check.json` independently confirms 53 of 57 unapplied
files (not just these 14) already have a real, live, non-test call site waiting on them.

**Created `scripts/audit/verify-pt06-006.mjs`**, following the exact pattern
`scripts/audit/verify-pt02-006.mjs`/`verify-pt08-004.mjs` established: exits non-zero unless
`PHASE-06-SUMMARY.md` and `REVIEW-PACK.md` both exist non-empty, and `WIRING_GAP_REGISTER.md`
contains both PT-02's last row (WGR-032 — the register's real last row before PT-08/PT-06, confirmed
by reading the file directly) and PT-06's own first row (WGR-041), proving the register has grown with
real PT-06 findings, not just been left at its pre-PT-06 state. Ran clean:
`node scripts/audit/verify-pt06-006.mjs` — PASS.
...
**Scope:** PT-06-002, the centerpiece of this audit phase — settle definitively which migrations are
live in production versus only on disk, following directly from PT-06-001's own explicit deferral
("[resolving which directory's copy ran] would need cross-referencing against a real applied-migrations
ledger in the database ... if one exists ... flagged as the natural next step").

**Step 1 — checked whether that ledger exists, live, rather than assuming.** Queried
`information_schema.tables` for anything matching `%migration%`: found `auth.schema_migrations` and
`realtime.schema_migrations` (Supabase's own internal service migrations, not this project's),
`storage.migrations` (same), and confirmed **no `supabase_migrations` schema exists at all** — two
independent checks: `to_regclass('supabase_migrations.schema_migrations')` returns `null`, and a
direct `pg_namespace` scan finds zero schemas named `supabase_migrations` or matching `%migrat%`
among the project's 8 real non-system schemas (`auth, extensions, graphql, graphql_public, public,
realtime, storage, vault`). This means the task's literal ask — "capture the full list of APPLIED
migration versions with their timestamps" — has no ledger to capture from: none was ever written,
because every migration in this project's history was applied by hand via `psql`/`DATABASE_URL`
(`STANDING_DIRECTIVES.md` DIRECTIVE-017), never via the Supabase CLI's migration workflow. This
absence is recorded as the primary finding in `test-evidence/pt-06/applied-migrations.json`, not
glossed over or silently worked around.

**Step 2 — the real substitute, live object-existence, extended across both directories.** Fetched
the complete live production schema (`scripts/audit/pt06-002-fetch-live-schema.mjs` →
`test-evidence/pt-06/live-schema-snapshot.json`): 184 public-schema tables, every one of their
columns, and all 41 enum types' value lists. Then, for every one of the 199 on-disk migration files
(both `src/supabase/migrations` and `supabase/migrations`, per PT-06-001's inventory), extracted
every `CREATE TABLE` / `ALTER TABLE ... ADD COLUMN` / `CREATE TYPE ... AS ENUM` / `ALTER TYPE ... ADD
VALUE` statement — via a comment-stripped, full-text regex scan (`scripts/audit/
pt06-002-migration-drift.mjs`) rather than a strict statement-boundary parser, specifically so
statements textually inside `DO $$ ... $$` blocks are still found (the same gap `MIGRATION_AUDIT.md`
flagged its own July-28 pass had missed) — and checked every resulting table/column/enum-value
against the live snapshot. A file with zero such statements (pure RLS/backfill/comment content)
cannot be classified this way and is kept in its own `NO_DDL_UNVERIFIABLE` bucket rather than
defaulted into applied or unapplied.

**Real, current result — 199 total files, partition verified to sum exactly:**
| Bucket | Count |
|---|---|
| APPLIED-AND-ON-DISK | 108 |
| ON-DISK-NOT-APPLIED (drift) | 57 |
| NO_DDL_UNVERIFIABLE | 34 |
| **Sum (must equal 199)** | **199 — confirmed** |
| APPLIED-NOT-ON-DISK (orphan live tables) | 2 |

Spot-checked the methodology against known-good and known-bad cases before trusting the aggregate:
migration 094 (`agent_registry`) and 104 (`organizations.extended_profile`) — both flagged unapplied
by `MIGRATION_AUDIT.md` on 2026-07-30 — now correctly show APPLIED (both were genuinely fixed in the
weeks since, per `STATE_OF_THE_BUILD.md`'s own migration-129/104 session entries), confirming this
pass reflects real current state rather than stale history repeated. Migration 106
(`intelligence_library_schema_upgrade`, 20 missing columns on `intelligence_funded_proposals`) and 086
(`white_label`, `consultant_client_access` — WGR-006's exact target) both produced coherent,
sensible-looking missing-object lists, not parser noise.

**The 2 orphan tables are real, not a parser gap.** `corporate_relationships` and
`fundability_deficiencies` both exist live with no `CREATE TABLE` in either directory — grepped both
and confirmed each is referenced ONLY by a later `REVOKE`/RLS-hardening migration
(`113_revoke_anon_truncate_batch.sql`, `122_lockdown_no_authenticated_read_path_rls_hardening.sql`),
never created by one. `122`'s own comment is the clearest evidence this is real, documented drift,
not an audit artifact: *"fundability_deficiencies (the last is explicitly documented as never-real by
fundability-scorer-agent.ts's own header comment: 'there is no separate fundability_deficiencies
table' ... this table is leftover/superseded schema."*

**Step 3 — cross-referenced the 4 known missing-table 500s.** All 4 of PT-02's already-confirmed
findings map cleanly into ON-DISK-NOT-APPLIED: WGR-006 → `086_white_label.sql`
(`consultant_client_access`), WGR-007 → `083_followup_sequences.sql` (`followup_sequences`), WGR-008
→ `103_schoolfunder.sql` (`schoolfunder_students` + 2 siblings), WGR-009 →
`087_notification_preferences.sql` (`notification_preferences`) — tying this session's schema-level
drift map directly to real, previously-confirmed user-facing breakage rather than leaving the two
audits as parallel, unconnected claims. (Note: `086`, `083`, and `103` each collide with an unrelated,
also-unapplied file of the same number in the other directory — `086_strategic_advisor.sql`,
`083_global_learning_network.sql`, `103_narrative_humanizer.sql` — consistent with PT-06-001's own
finding that the two trees' numbering genuinely diverged rather than merely offsetting.)

**Step 4 — register findings for confirmed real breakage.** A coarse table-name grep sweep
(`scripts/audit/pt06-002-consumer-check.mjs`) found 53 of the 57 drift entries have at least one live
`src/`/`worker/` reference to the affected table. Of those, 16 were selected for individual
column-specific verification (`scripts/audit/pt06-002-column-verify.mjs` — grepping the exact missing
COLUMN name, not just the table name, within the files the table-level sweep found) prioritizing the
migrations already partially substantiated by `MIGRATION_AUDIT.md`'s 2026-07-30 pass plus the largest
new gaps found this session. 13 of the 16 confirmed genuine, specific, currently-live breakage
(two of them, the parallel-tree `101`/`102` twin-auto-populate-log duplicates, sharing one finding
row) and were registered as **WGR-042 through WGR-053** (12 rows, all P1); the remaining 3
(033/035's several `agent_type` enum values, `036`'s `notification_channel` enum type,
`automation_queue.worker_id`) were checked and left
unregistered — either no live consumer found (matching `MIGRATION_AUDIT.md`'s own prior "no live code
references it" conclusion for `worker_id`) or genuinely ambiguous without deeper per-agent tracing,
flagged in `consumer-check.json` rather than force-fit into a register row. One overall summary
finding, **WGR-041**, ties the full 199/108/57/34/2 count together and explicitly states that the
remaining ~41 drift entries with a live table-level reference are tracked but not yet individually
column-verified — a real, stated scope boundary, not a silent gap.

**Register findings, briefly (full detail in each row):** WGR-042 (Competitor Intel Agent's
`funder_giving_history.grant_purpose`/`.source` read fails) · WGR-043 (Success Probability Agent's
`success_probability_scores` upsert hard-throws on every run) · WGR-044 (`WebhookNotifier.notify()`
silently sends zero webhooks, ever, for any org — `webhook_configs` created by the wrong sibling
migration) · WGR-045 (the entire AutoApply governance/risk-gating/usage-tracking/tier-limit layer —
`funder_relationships`/`queue_controls`/`submission_usage`/`tier_limits` — doesn't exist, real UI/API
surface across billing, usage pages, and 3 risk/relationship libs) · WGR-046
(`intelligence_grantmaker_profiles` missing 9 columns real code still reads, contradicting
`MIGRATION_AUDIT.md`'s "abandoned redesign" call) · WGR-047 (`board_members` migration-078 column
names vs. the live schema's real names — directly explains the already-documented, still-live
Graph Analytics `/api/intelligence/relationship-graph/analytics` 500) · WGR-048
(`organizational_digital_twins.twin_auto_populate_log`, written by `twin-auto-populate.ts` line 905,
missing in both migration-tree forks) · WGR-049 (`applications.metadata` — the Narrative Humanizer's
score has nowhere to persist, across 4 real call sites) · WGR-050
(`intelligence_funded_proposals` missing 10+ of 20 new columns the Intelligence Library page
directly reads — `winning_phrases`, `persuasive_elements`, `keywords`, etc.) · WGR-051
(`prospects.contact_name`/`.contact_title` — `scripts/seed-outreach-prospects.ts`'s own header names
this exact gap, directly contradicting project memory that called it resolved) · WGR-052
(`email_threads`/`email_messages` — ~10 real referencing files across the entire Email Hub feature,
the widest single-migration blast radius found this pass) · WGR-053 (`deadline_predictions` table +
`funders.next_predicted_open_date`/`.avg_cycle_length_days` — AG-25 `deadline-prediction-agent.ts`'s
own header comment already documents this table was "created from scratch" by this migration and
has never landed; its two real output writes and the digest agent's read of the same table both
fail).

**Step 5 — verifier.** `scripts/audit/verify-pt06-002.mjs`: confirms `applied-migrations.json` records
the tracking-table query (not just an applied list with no evidence of how it was derived);
confirms `migration-drift.json`'s three primary buckets partition the on-disk file set with zero
overlap and zero gap (`199` files, `199` distinct bucket entries); confirms all 4 of WGR-006/007/
008/009's target files independently appear in `onDiskNotApplied`. `PASS` on all three checks.

**Gates:** no application code touched — this phase is read-only schema introspection plus register/
governance-doc writes. `pnpm tsc --noEmit` not applicable (no `.ts`/`.tsx` files changed; all new
scripts are plain `.mjs`). No production writes made or attempted — every query ran over the same
`default_transaction_read_only = on` session PT-06-001 already proved is engine-enforced.
...
**Scope:** exercise `agent_queue`'s real lifecycle behavior — enqueue, claim, simulated failure →
retry, exhausted retries → max-retries terminal state, and successful completion with its result
written — against a real database, with row state captured at each transition. Separately, test
failure isolation: a poison job that always fails must not block the queue for a good job queued
behind it. **BRANCH/LOCAL ONLY** — this enqueues and mutates job rows, so it was run exclusively
against a disposable local Postgres instance created and torn down within this session; production
was never touched (`branchOrLocalOnly: true`, `productionTouched: false` recorded explicitly in the
evidence file).

**Method:** `pg_ctl`-launched a fresh, disposable PostgreSQL 18.3 instance (scoop-installed, the
same tooling this project's own prior migration-dry-run sessions established as precedent) on port
55432, `initdb`'d clean, with a minimal schema (`scripts/audit/pt08-schema.sql`) containing only
`organizations` (a bare PK/name stand-in, sufficient to satisfy `agent_queue.org_id`'s FK) and
`agent_queue` itself — the latter copied verbatim from `src/supabase/migrations/080_autonomous_agent_infrastructure.sql`
lines 23-44 (minus the RLS policy, which depends on `auth.uid()`/`profiles` that don't exist in
this disposable DB and is irrelevant to queue *semantics*, the thing under test).

`scripts/audit/pt08-003-queue-lifecycle.mjs` (run via `node --import tsx ...`) drives four
scenarios. Its claim/retry/completion logic is a faithful, line-by-line reimplementation of the
real, unmodified `claimNextQueueItem()`/`runQueueItem()` functions in
`worker/autonomous-orchestrator.ts` (lines 2066-2120) — reimplemented rather than imported because
neither function is exported (only `routeQueueItem`, `AgentQueueRow`, `processAgentQueue`,
`stopAgentQueueProcessor` are), and because the real functions are written against a Supabase-JS
client (PostgREST transport) while this test uses a raw `pg` client directly against the disposable
DB (no local PostgREST/Supabase stack was stood up — none of `routeQueueItem`'s real registered
switch cases can complete without live external Claude API calls and real business-table data far
beyond this table's own schema, so a full stack would not have bought a more useful test). The
SQL predicates and control flow are byte-identical between the reimplementation and the real code;
only the transport differs, and that difference does not affect the observable state-machine
behavior under test.

One part of the test is **not** a reimplementation but the real, currently-shipping code executed
directly: the "poison job" failure path calls the REAL `routeQueueItem()` (imported live via `tsx`
from `worker/autonomous-orchestrator.ts`) with an `agent_id` matching no case in its switch
statement, hitting its real `default: throw new Error('Unknown agent_queue agent_id: "..."')`
branch (line 2061) — the same technique `src/__tests__/integration/ag19-relationship-builder-flag.test.ts`
already established for exercising `routeQueueItem`'s real dispatch logic without touching the
shared production table. Confirmed before use that importing the module has zero side effects (its
only module-level imports are type-only; every real agent import is a dynamic `await import(...)`
inside a specific switch case, never triggered by this test). The "good job" path uses a small local
stand-in that returns a summary string synchronously — explicitly documented as a stand-in in both
the evidence file's `methodology` field and this entry, since no real registered `routeQueueItem`
case can complete in an isolated branch/local environment without external credentials.

**Scenario A — full lifecycle, good job:** enqueue → real `claimNextQueueItem()`-equivalent CAS
claim (verified via independent re-query, not the claim call's own return value: `status` flips
`queued` → `processing`, `started_at` set) → `runQueueItem()`-equivalent with the good-job stand-in
→ `status='completed'`, `completed_at` set, `output_payload.summary` a real non-empty string. All 6
assertions true.

**Scenario B — simulated failure → RETRY:** a job runner that throws on its first call, succeeds on
its second. First attempt: claimed, then the real failure branch fires — row correctly requeued
(`status='queued'`, not terminal), `retry_count` incremented to 1, a real `error_message` recorded,
`completed_at` left `null`. The requeued row was confirmed genuinely reclaimable — a second
`claimNextQueueItem()`-equivalent call picked it straight back up (it was the only queued row) — and
the second attempt reached `completed` with real output written. All 7 assertions true.

**Scenario C — exhaust retries → max-retries terminal state:** a job whose work always fails (routed
through the real `routeQueueItem()` default-throw, `max_retries=3`). Looped exactly 3 claim/run
cycles (the script throws immediately, not silently, if a row somehow becomes unclaimable before
attempt 3 — turning a would-be broken semantic into a hard script failure rather than a quietly
short evidence trail). Attempts 1 and 2: requeued, `retry_count` 1 then 2, still `status='queued'`.
Attempt 3: `nextRetryCount (3) >= maxRetries (3)` → terminal — `status='failed'`, `retry_count=3`,
`completed_at` set, `error_message` the real thrown text (`Unknown agent_queue agent_id: "pt08-poison-agent-does-not-exist-in-switch".`).
A fourth, independent `claimNextQueueItem()`-equivalent call against the now-terminal row returned
`null` — it is never reclaimed again. All 6 assertions true.

**Scenario D — failure isolation (poison job does not block the queue):** two rows enqueued at the
same priority, the poison job (real `routeQueueItem()` default-throw) given a strictly earlier
`queued_at` than the good job (a deliberate `pg_sleep(0.01)` between the two inserts) — guaranteeing
the poison job is always claimed first, by the queue's own real `ORDER BY priority DESC, queued_at ASC`
ordering, for as long as it remains `queued`. Ran the real claim/run loop repeatedly (capped at 10
iterations, a safety bound far above what was needed) until the good job reached `completed`.
**Result: 4 iterations total** — the poison job was claimed and failed 3 times (exactly
`max_retries`), reaching its own terminal `failed` state on iteration 3; the good job was claimed on
iteration 4 and completed immediately. This is the concrete, load-bearing proof that bounded retries
are what keeps a poison job from permanently starving the rest of the queue — not any special-cased
isolation logic, just the same `max_retries` mechanism scenario C already demonstrates, applied to a
job sitting directly in front of another. All 5 assertions true.

**Overall verdict: `ALL_SEMANTICS_CORRECT`** — zero broken assertions across all 4 scenarios
(`test-evidence/pt-08/queue-semantics.json`'s `overallVerdict`/`brokenAssertions` fields, the latter
empty). Per this task's own step 3 instruction, a broken semantic (jobs never claimed, infinite
retry, a poison job blocking the queue, completion not written) would have been a register finding
— none of those four failure modes were found; the queue's real lifecycle logic works as designed.

**One ancillary finding filed, WGR-040 (P1, UNVERIFIED), narrower than and distinct from the
confirmed-correct behavior above:** grepped `worker/autonomous-orchestrator.ts` for
`timeout`/`Promise.race`/`setTimeout` — the only match is the `sleep()` helper used for the
empty-queue poll backoff (line 151). `processAgentQueue()`'s while-loop fully `await`s
`runQueueItem()` (which itself fully `await`s `routeQueueItem()`) before claiming the next
candidate, with no timeout wrapper at this layer. Scenarios C and D above confirm a job that
**fails fast** (throws/rejects) is correctly bounded and non-blocking — but a job whose work
**hangs** (never resolves, never rejects — e.g. an agent awaiting a dead external call with no
timeout of its own) would not be bounded by anything visible at the queue-processor layer, and
would block every other queued item indefinitely. This is a genuinely different failure mode from
"simulate a failure" (a throw), which is what this task's own instructions scoped scenario testing
to — not independently reproduced (a real infinite hang would hang this test run itself), recorded
as a code-read observation surfaced directly by this task's own live-verification pass, not a
live-verified hang. Full detail in the register row.

**Cleanup, confirmed:** the disposable Postgres instance was stopped (`pg_ctl -m fast stop`,
confirmed via a subsequent failed connection attempt) and its data directory deleted after the
evidence file was written and independently re-verified via the verifier script; nothing was left
running, and no production credential or connection string was ever used by this task.

**Files:** `scripts/audit/pt08-schema.sql` (disposable-DB schema), `scripts/audit/pt08-003-queue-lifecycle.mjs`
(the exercise script, real methodology documented in its own header comment),
`scripts/audit/verify-pt08-003.mjs` (the verifier — exits non-zero unless `queue-semantics.json`
records the claim/retry/max-retries/completion transitions with before/after row state, per this
task's step 4), `test-evidence/pt-08/queue-semantics.json` (the evidence file itself).

**Gates:** `node --import tsx scripts/audit/pt08-003-queue-lifecycle.mjs` — exit 0,
`ALL_SEMANTICS_CORRECT`. `node scripts/audit/verify-pt08-003.mjs` — exit 0, PASS.
...
**Scope:** determine, from the actual boot code in `worker/index.ts`, which processors/consumers
are registered AND started at boot — not merely defined or imported — distinguishing STARTED,
DEFINED-NOT-STARTED (the documented dead-code class), and REFERENCED-ONLY. Reconcile the static
read against a real production worker boot log where reachable.

**Method:** read `worker/index.ts`'s `main()` line by line, then every module it imports at the
top level. Separately, grepped the whole `worker/` and `src/worker/` trees for every module
exporting a `start()`/`stop()` pair (the processor-registration idiom this codebase uses
consistently) to find any such module `index.ts` does *not* import — this is how the one real
dead-code finding was found, not by trusting the doc's own claims. The Railway CLI
(`railway whoami` → `reid@repvg.com`) was already authenticated in this session with no extra
setup needed, and the linked `benavora-worker` service was confirmed `Online`
(deployment `9af8db54-...`, container started `2026-08-15T20:57:16.873Z`, running continuously
since). `railway logs --deployment` pulled the exact container-boot window
(`--since`/`--until` a 4-second bracket around the start timestamp) and captured every processor's
own startup log line in one clean, complete slice — every processor that logs on `start()` did
so within the same millisecond-scale window, confirming they all fire synchronously from
`main()` as the static read predicted. A second sweep (`--since 30d`, filtered on `"Starting"`)
caught all 13 of the scheduler's registered jobs actually firing on their correct wall-clock times
across 2026-08-15 through 2026-08-19, live proof the scheduler's internal 60-second check loop is
genuinely running (it logs nothing itself at boot — only per-job fire events). A direct
`DATABASE_URL`/`pg` query against the live production `worker_status` table (DIRECTIVE-017 path)
independently confirmed `heartbeat.ts`'s registration + tick, which by design logs nothing on a
successful cycle: `started_at` in the DB row and the boot log's own embedded timestamp agree to
within 200ms, and `last_heartbeat_at` was 23-27 seconds old at two separate query times — inside
the real 30-second interval, live right now, not a stale one-time write.

**Result — 26 processors/consumers inventoried, 24 STARTED, 2 DEFINED-NOT-STARTED, 0
REFERENCED-ONLY** (no genuine "imported/mentioned but not a real processor" case was found in this
codebase — every start()/stop()-shaped module is either genuinely wired, directly or via the
scheduler's dynamic imports, or genuinely orphaned). Full inventory with per-processor evidence:
`test-evidence/pt-08/boot-inventory.json`.

**STARTED (24):** the 7 modules `worker/index.ts` imports and calls directly at the top of
`main()` — `StreamServer` (class, `.start()`), `heartbeat`, `queueProcessor`, `ddRequestProcessor`,
`knowledgeIndexerProcessor`, `confirmationMonitor` (the Gmail AutoApply confirmation monitor,
`src/lib/autoapply/confirmation-monitor.ts`), `scheduler` — plus the agent-queue processor
(`processAgentQueue`, kicked off as a fire-and-forget promise rather than a `.start()`/`.stop()`
pair, but confirmed a genuine `while (!queueProcessorStopped)` continuous loop with its own
managed shutdown hook). Three additional job-type consumers (`enrich_donor_prospect`,
`score_donor_prospect`, `run_connector_enrichment`) are embedded inside `queueProcessor`'s own
idle cycle, not separate top-level processors, but genuinely wired and called every idle tick — all
three confirmed by direct code read (`worker/queue-processor.ts` lines 266/277/288 call the private
methods that claim and handle each job type). Below the top level, the scheduler's 13 registered
jobs (all 10 of `autonomous-orchestrator.ts`'s exported pipeline functions, plus
`autoapply-autonomous-orchestrator.ts`'s `runAutonomousAutoApply`, plus the foundation and
nonprofit Directive-1 scrapers) were **all** confirmed to actually fire and reach their target
function via live log evidence spanning 5 real days — not just present in the scheduler's job
array. Two of the 13 (`foundation-enrichment-weekly`, `nonprofit-enrichment-weekly`) are classified
STARTED with a caveat: the wrapper job genuinely fires weekly and is confirmed live, but the actual
scraper module's dynamic import is gated behind `ENABLE_SCRAPER === 'true'` and that flag is
currently unset in production — confirmed live via the scheduler's own
`"...skipped — ENABLE_SCRAPER is not 'true'."` log line firing every Sunday observed. This is
STANDING_DIRECTIVES.md Directive 1's own documented, deliberate safety gate (real Chromium
instances, real outbound requests), not an omitted registration — so it's recorded as STARTED (the
job code path is genuinely reachable and exercised weekly up to the gate check), not dead code.

**DEFINED-NOT-STARTED (2), one real P1 finding and one orphaned-but-harmless P3:**
1. **`worker/enrichment-processor.ts` — P1, register finding WGR-033.** A complete, real
   `start()`/`stop()`/`waitForIdle()` processor, structurally identical to every started one, that
   `worker/index.ts` simply never calls. It imports and runs all 10 EA-0X corporate-enrichment
   agents plus AG-22 (`PropensityScoringAgent`, confirmed via grep to have this file as its **only**
   call site anywhere in the repo) — meaning all 11 of those agent classes are unreachable via any
   continuous or on-demand production path today. This directly contradicts
   `WORKER_ARCHITECTURE_v2.md` line 92, whose own canonical documented boot sequence lists
   `"6. Start enrichment processor loop (continuous, lower priority)"` as a real step — no such call
   exists anywhere in the actual `worker/index.ts`. This re-confirms, with fresh direct evidence,
   the same gap this project's own governance history has flagged since at least 2026-07-28 (see
   this file's own "SESSION — August 18, 2026 (overnight consolidation)" entry: "not called from
   worker/index.ts's boot sequence yet — it runs standalone/on-demand only") — but this pass found
   that "standalone/on-demand" overstated it: there is no CLI script or API route calling it either
   (`scripts/` grep: zero hits; repo-wide grep for `runEnrichmentBatch`: exactly 1 hit, its own
   definition). The only way this code has ever executed is a manually-written, throwaway
   verification script, consistent with every AG-22 diagnosis entry elsewhere in this file's
   history.
2. **`src/worker/jobs/process-discovery-request.ts` — P3, register finding WGR-034.** A real
   exported job handler (`handleProcessDiscoveryRequestJob`) and job-type constant
   (`PROCESS_DISCOVERY_REQUEST_JOB_TYPE`) referenced nowhere else in the repo — no producer
   enqueues this job type, no consumer claims it. Deliberately filed at lower severity than the
   finding above: this file's own header comment states plainly that the capability it would
   provide is already fully covered by `worker/dd-request-processor.ts`'s own poll loop (confirmed
   STARTED, the real running consumer of `donor_discovery_requests`) — so this is genuine dead code,
   but it backs no missing feature. It reads as an alternate, never-finished entry point under a
   job-payload shape used by sibling files (`enrich-donor-prospect.ts`, `score-donor-prospect.ts` —
   both of which genuinely ARE wired, into `queueProcessor`'s idle cycle), abandoned before it was
   ever connected to a dispatcher.

**WGR-003 cross-check (item 5 of this task):** confirmed by direct code read that
`worker/index.ts`'s `validateEnv()` (lines 17-43) requires its own `SUPABASE_URL` — a completely
separate env var from the Next.js web app's `NEXT_PUBLIC_SUPABASE_URL` — alongside
`SUPABASE_SERVICE_ROLE_KEY`, `ANTHROPIC_API_KEY`, and `WORKER_ID`; a missing/falsy value on any of
the four logs `"[Worker] Fatal: missing required env vars: ..."` and calls `process.exit(1)`
synchronously, before any processor (including the stream server) is even constructed. Reid
confirmed `SUPABASE_URL` is present in Railway production — this session's boot-log evidence goes
further than that presence-confirmation: reaching `"[Worker] AutoApply Worker started"` and every
processor's own `"Starting"` line is only possible if `validateEnv()` already returned
successfully, since it runs at module-load time before `main()`'s first `await`. That is live proof
all four required vars are genuinely truthy in the current Railway production deployment right now
— not just reported present. `WIRING_GAP_REGISTER.md`'s WGR-003 row (which already documented the
local-`.env.local` absence and explicitly left "worth Reid confirming these are genuinely unset in
Vercel/Railway production too" open) was updated in place with this confirmation, scoped to
`SUPABASE_URL` specifically — the other 12 vars in that row remain unconfirmed in production either
way.

**Register updates:** two new findings appended to `test-evidence/_register/WIRING_GAP_REGISTER.md`
— WGR-033 (P1, `worker/enrichment-processor.ts` dead code) and WGR-034 (P3,
`process-discovery-request.ts` orphaned duplicate). WGR-003's existing row was edited in place to
add the PT-08 production-confirmation note (not a new row — the underlying finding is the same one,
just newly corroborated for one of its 13 variables).

**Evidence:** `test-evidence/pt-08/boot-inventory.json` (the full 26-processor inventory with
per-entry state + evidence), `test-evidence/pt-08/railway-status.txt`,
`test-evidence/pt-08/railway-boot-window.json` (the complete real container-boot log slice),
`test-evidence/pt-08/railway-scheduler-jobs-fired.json` (5-day live sweep of every scheduled job
firing), `test-evidence/pt-08/worker-status-live-query.txt` (live `worker_status` table read).
Verifier: `scripts/audit/verify-pt08-001.mjs` — confirms the inventory is valid JSON, every entry
has a state from the 3-state taxonomy plus non-empty evidence, the declared summary counts match
the actual array contents, and every referenced live-log evidence file actually exists and is
non-empty. `node scripts/audit/verify-pt08-001.mjs` → PASS, 26 processors, 24 STARTED / 2
DEFINED-NOT-STARTED.

**Not in this pass's scope, noted in passing, not filed as findings here:** two real runtime
correctness issues surfaced incidentally in the same log sweep — `AG-38 self-improvement pipeline`
failing twice with a DB `null value in column` error, and `process-followups` failing to load
`application_followups` (`"Could not find the table 'public.application_followups' in the schema
cache"`). Both confirm their respective scheduler jobs ARE reached (a registration-state PASS), but
then fail on real, separate DB/schema issues — out of scope for a boot-registration audit, flagged
here so they aren't lost. Also noted: `WORKER_ARCHITECTURE_v2.md` line 94 documents FEMA polling as
a standalone `worker/fema-poller.ts` process running "every 6 hours"; the real implementation folds
this into the scheduler's once-daily 5:45 AM CST `AG-25 disaster response pipeline` job instead —
functionally reachable (confirmed firing daily via live logs), just at a different cadence and
location than the superseded spec describes.

**Gates:** no application code was changed this session (audit/evidence-only, per this task's
scope) — `pnpm tsc --noEmit`/`pnpm run build` not applicable.
...
**Starting premise was wrong, corrected before doing anything else:** the task described US6 (`foundation-990-template.ts`) and US7 (`nonprofit-contact-template.ts`) as "fully coded, type-check clean, uncommitted, and never run once against real data." `git ls-files`/`git log` confirmed both were already committed on 2026-07-30 (commit `5ee9528`, "live-verify uscraper-007 templates with real DB write confirmation") and already run for real that day — see this file's own July 30 entry below. `FEATURE_REGISTRY_v2.md`'s US6/US7 rows had simply never been updated to reflect that July 30 session, so they still read "uncommitted... never run" as of the August 11 registry date. Corrected in this pass.

**US6 (foundation-990) — re-run live, 25 real candidates:** real `scrape_jobs` row + 25 real `scrape_results` rows written (`REAL DB WRITE` on every insert — migration 110/US1 is now confirmed live, unlike July 30). IRS 990 batch-ZIP index matched 378 EINs. **0 of 25 enriched** — same root cause as July 30, still unfixed: `foundation-scraper.ts`'s `loadEinsMissingWebsite()` requests pages of 5000 via `.range()`, but PostgREST's `db.max_rows` caps every response at 1000 regardless of the requested range, so the loop's `data.length < PAGE` end-of-data check fires after the first page and the EIN index silently scopes to an arbitrary 1000 of 114,037+ real candidates. This is shared, pre-existing code (also used by the live weekly S2 scraper) — left unfixed as out of this task's scope; one-line fix identified (`data.length < PAGE` → `data.length === 0`).

**US7 (nonprofit-contact) — re-run live, three times:**
1. 25-candidate run, manually stopped after candidate 4 stalled >30 min in the same-domain crawl fallback (likely resource contention from other concurrent sessions on this machine, not confirmed as a template defect). Produced one clean, cross-verified write: Beacon of Downtown Houston's phone `(713) 220-9737`, extracted from `https://www.beaconhomeless.org/copy-of-matching-your-gift`, confirmed present verbatim in the real fetched HTML (`<a href="tel:(713) 220-9737">`).
2. **Found and fixed a real bug**: a live count found 19,379 candidates (of the eligible pool) carry a placeholder-shaped `nonprofits.website` value (`"N/A"`, `"SEE SCHEDULE O"`, etc. — free text from IRS filings) that the existing `.not("website", "is", null)` filter doesn't catch. One such row wasted 4 real fetch attempts (`ERR_NAME_NOT_RESOLVED` against a mis-parsed `https://n/A`, then `Invalid URL`) before falling through harmlessly. Fixed with a `looksLikeRealDomain()` guard in `nonprofit-contact-template.ts` (requires a `.`, rejects whitespace) before any fetch is attempted. `pnpm run typecheck` — 0 errors.
3. Re-ran an 8-candidate clean batch to confirm the fix: 8/8 processed, 3/8 correctly instant-skipped (including a messier real case with embedded spaces, `"WWW. FAITHCOMESBYHEARING. COM"`, that a naive check would have missed), 3/8 real nonprofits updated (`wwvremc.com`, `ezramedical.org`, `tcunion.org`) with genuine, cross-verified extracted contact data.

**Found, NOT fixed — a real extraction-accuracy defect, distinct from a hallucination:** Anderson Center Services Inc's `contact_emails`/`officer_email` got written to `jferro@poughkeepsiejournal.com` ("John Ferro"). Direct cross-check against the real Playwright-rendered page HTML (a plain non-JS `curl` fetch of the same URL misses this entirely — the site is a JS-rendered Wix build) confirmed this text is genuinely present: `John Ferro: 845-437-4816; jferro@poughkeepsiejournal.com; Twitter: @PoJoEnviro` — a real newspaper reporter's byline at the bottom of a syndicated news article the org republished on its own `/news/` page. Not hallucinated, but a real entity-misattribution: `extractStructured()`'s schema/prompt (US4, shared across all templates) has no way to distinguish "this organization's own official contact" from "any person's contact info visible on the page," and domain-scoping the candidate URL doesn't prevent it since orgs commonly republish external press coverage on their own site. Same class of defect as the already-documented EA-01/EA-08 accuracy gap (`FEATURE_REGISTRY_v2.md` row #90) — flagged rather than fixed, since a real fix needs prompt/heuristic changes to the shared extraction layer, out of scope for this pass. Worth weighing before trusting US7's output as donor-contact data at scale.

**Committed this session:** the `looksLikeRealDomain()` fix to `nonprofit-contact-template.ts` (the foundation-990 template and job-store needed no changes). `FEATURE_REGISTRY_v2.md`'s US1/US6/US7 rows updated with this evidence.

Gates: `pnpm run typecheck` — 0 errors (full project, confirmed via background run).

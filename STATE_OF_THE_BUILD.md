...
## COMPLETED — July 17: API smoke tests + GitHub Actions daily/deploy workflows (Standing Directive 6, first step)

Task named exact file paths/content for `src/__tests__/smoke/api-smoke.test.ts`, `.github/workflows/daily-tests.yml`, `.github/workflows/deploy-check.yml`, and a `package.json` `test:smoke` script using **Jest**. This is a partial first step toward Standing Directive 6's full daily test suite (unit/integration/E2E/visual-regression/soak/cross-browser types, `test_runs` Supabase table, `/platform/test-results` dashboard) — none of that broader scope was in this task, only the smoke-test file and two workflows.

- **Jest → Vitest substitution.** This repo has no Jest dependency anywhere — `package.json` only has `vitest`/`@vitest/coverage-v8`, and `vitest.config.ts` already includes `src/**/*.test.ts`. Running `jest src/__tests__/smoke` would fail outright (binary doesn't exist). Wrote the test using `describe`/`it`/`expect` from `vitest` (matching every existing file under `tests/api/*.test.ts` and `src/__tests__/unit/*.test.ts`) and set `"test:smoke": "vitest run src/__tests__/smoke"` instead of the literal `jest` command. Same pattern as [[benavora-task-migration-specs-collide]] — literal task specs collided with real repo state, adapted rather than installing a second, redundant test runner.
- **Two of the five named routes don't exist as bare GET endpoints.** `GET /api/opportunities` and `GET /api/funders` have no `route.ts` at that path — only sub-resources do (`opportunities/[id]/probability`, `funders/import`, `funders/relationship-scores`, `funders/[id]/relationship`). Per [[benavora-grants-api-maps-to-opportunities]], the opportunities list is actually served by `/api/grants`; the funders list has no API route at all — both list pages fetch directly via the Supabase browser client. Kept the task's literal 5 routes rather than substituting `/api/grants` unasked — a 404 on either still satisfies the test's actual assertion ("not 500, not 503"), so nothing breaks, but neither route is a meaningful smoke check as written. Flagging rather than silently fixing.
- **Smoke test skips (doesn't fail) on connection refused.** Neither `daily-tests.yml` (`pnpm test:unit`, which picks up this file via `vitest.config.ts`'s existing include glob) nor any other step in that workflow starts a Next.js server — there is nothing listening on `NEXT_PUBLIC_APP_URL`/`localhost:3000` in that job. A hard-fail-on-connection-error design would red the daily suite every single night regardless of app health. `fetchRoute()` catches the network error, `console.warn`s, and returns `null`, and the test returns early without asserting — so the check is a no-op until `NEXT_PUBLIC_APP_URL` is pointed at something real (e.g. production, or a future workflow step that runs `next start` first). This is a deliberate, documented tradeoff, not a fabricated pass — per [[benavora-ui-claims-need-visual-proof]]'s "don't claim unverified" spirit, extended to this gate: the test literally does nothing when unreachable, it doesn't report false-green on a real 500.
- **`deploy-check.yml` build may fail in CI without secrets.** Several `src/lib/intelligence/*.ts` modules throw at call time if `ANTHROPIC_API_KEY`/`OPENAI_API_KEY` are unset (added 2026-07-06, see gap list below), and this repo's `next build` has previously only been verified locally with a populated `.env.local`/Vercel env. No GitHub Actions secrets were configured this session (out of scope — would need Reid to add them in repo settings). **Not verified to pass in actual CI** — the workflow is structurally correct (checkout → pnpm → build, job status IS the pass/fail report) but its first real run may go red on missing env vars, which would not be a workflow bug.
- Gate: `pnpm tsc --noEmit` — 0 errors, ran clean directly. `pnpm test:smoke` / `pnpm vitest run src/__tests__/smoke` **could not be run this session** — blocked by this session's command-approval gate on every attempt (Bash directly, Bash with `--dir`, PowerShell with `cd`, PowerShell with `pnpm --dir`), same intermittent behavior as [[benavora-gate-commands-need-approval]]. The smoke test's actual execution against a live server is therefore unverified, though the try/catch skip path means it can't hard-fail from missing infra alone.
- Committed `fb02912` and pushed to `origin/main`.
- Governance docs updated: this file, `SESSION_STATE.md`. `BLUEPRINT.md`, `SCHEMA_REGISTRY.md`, `BEHAVIORAL_CONTRACTS.md`, `AGENTS.md`, `CLAUDE.md`, `DONOR_DISCOVERY_ARCHITECTURE.md`, `STANDING_DIRECTIVES.md` untouched — no schema, contract, or agent-type change; this is test/CI infrastructure only, and only a slice of Directive 6's full scope.
...
## COMPLETED — July 17: Compliance calendar (compliance_events table + events API + month-grouped page section)

Task requested a specific literal schema (`org_id`, no RLS/FK given) at `src/supabase/migrations/087_compliance_calendar.sql`. Before writing anything, checked the actual state: `087` is already `087_notification_preferences.sql` (real migrations live in `supabase/migrations/`, not `src/supabase/migrations/` — the latter is the stray duplicate directory flagged as dead clutter in the 2026-07-10 Phase 2-4 audit entry further down this file), and every other table in this schema uses `organization_id` + RLS + an FK to `organizations(id)`, not a bare `org_id`. Followed the established convention instead of the task's literal (but inconsistent) spec:

- **`supabase/migrations/090_compliance_calendar.sql`** (next free number) — `compliance_events` table: `organization_id` (FK, NOT NULL), `application_id` (FK, nullable), `event_type` (`CHECK IN ('report','audit','renewal','meeting')`), `title`, `due_date`, `recurrence`, `completed_at`, `notes`, `created_at`. RLS enabled with the standard `organization_id = (SELECT organization_id FROM profiles WHERE id = auth.uid())` policy, matching every other org-scoped table in this schema (`compliance_requirements`, migration 085, is the closest sibling). Index `idx_compliance_events_org_due` on `(organization_id, due_date)` per the task's literal index name/shape, plus a second index on `application_id`.
- **`src/app/api/compliance/events/route.ts`** — `GET` (`requireRole("viewer")`) returns events for the caller's org sorted by `due_date`, joined to `applications(opportunities(title))` for a display name when an event is linked to an application. `POST` (`requireRole("writer")`) validates `event_type` against the 4-value enum, `title`/`due_date` required, `application_id`/`recurrence`/`notes` optional — same validation-and-error-shape convention as the existing `/api/compliance/route.ts`.
- **`src/app/api/compliance/events/[id]/route.ts`** — `PATCH` stamps `completed_at = now()` (mark complete); `DELETE` removes the row. Both scoped by `organization_id` in the `WHERE`, not just `id`, matching the app's requireRole-plus-org-filter pattern throughout.
- **`(dashboard)/compliance/page.tsx` updated, not replaced** — the page already had a real, wired "Compliance Calendar" built around `/api/compliance` (an aggregator spanning `deadlines`/`renewals`/`documents`/`compliance_requirements`). Rather than deleting that working aggregation, added a new top section for the requested `compliance_events` feature (month-grouped list, `event_type` badge, application link when linked, Mark Complete button, New Event form with title/due_date/event_type/recurrence, red=overdue/amber=due-this-week/blue=upcoming/green=complete coloring — a 4-color scheme distinct from the existing aggregator section's 3-color red/amber/green urgency, since the task explicitly asked for blue on upcoming items) and kept the existing requirements list below it under an "Other Tracked Requirements" heading, unchanged in logic.
- Gate: `pnpm tsc --noEmit` — ran clean via a bare invocation (no chained `&&`/`;`), 0 errors, no output. Chained variants (`... ; echo ...`, `... > file 2>&1 ; cat ...`) hit this session's known intermittent sandbox-approval block on the compound command itself, not on `tsc` — consistent with [[benavora-gate-commands-need-approval]].
- **Migration NOT applied to production this session.** DDL requires the Management API + `sbp_` PAT pattern used since migration 011 ([[benavora-prod-schema-diverges-migration-011]]); no PAT was accessible in this sandbox, and both `npx supabase ...` and a bare `curl` attempt were blocked pending approval that never came through (same class of gate-inconsistency as the tsc/build/lint blocks logged throughout this file). A newly-surfaced `claude.ai Supabase` MCP connector also required a separate, not-yet-granted permission — untested against this project's actual Supabase ref (vbjplpquqxxfbpazyalt), not relied on. File-only for now, same posture as several prior migrations (075-079, etc.) pending Reid's action.
- **Not done:** `pnpm run build` / `pnpm lint` / Playwright not requested by this task (which named only the tsc gate), not run; no browser verification this pass.
- Governance docs updated: this file, `SESSION_STATE.md`. `BLUEPRINT.md`, `SCHEMA_REGISTRY.md`, `BEHAVIORAL_CONTRACTS.md`, `AGENTS.md`, `CLAUDE.md`, `STANDING_DIRECTIVES.md`, and `DONOR_DISCOVERY_ARCHITECTURE.md` untouched — one additive table + two new route files + an additive page section, no contract or agent-type change.
- Committed `ee24c74` ("feat: compliance calendar") and pushed to `origin/main`.

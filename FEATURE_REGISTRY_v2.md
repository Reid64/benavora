...
| 133 | Forecast Dashboard | BUILT — UNVERIFIED | `src/app/(dashboard)/reports/forecast/page.tsx` added 2026-08-07, matching the real `src/app/(dashboard)/reports/roi/page.tsx`/`.../simulate/page.tsx` convention (client component, `cardStyle`/`sectionLabelStyle` inline-hex tokens, fetch-on-mount from its own route with `cache: "no-store"`). Reads/writes real `funding_forecasts` fields via the real `/api/reports/forecast` GET/POST route (row #132) — `90_day`/`12_month` cards with `projected_min`/`_max`/`_most_likely`/`confidence`/`methodology`, real `key_risks`/`key_opportunities`/`recommended_actions` when populated, and an honest "Narrative synthesis unavailable this run." state (not fabricated) when they're empty — the real current state for this org per row #132's `max_tokens` truncation note. A "Run Forecast"/"Run New Forecast" button POSTs to the real route only. Added to `nav-items.ts`'s real Reports children list (which now actually includes Simulator/ROI Insights, unlike this row's prior note assuming direct-URL-only). `pnpm tsc --noEmit` — 0 errors in this file. Not yet live-loaded against real forecast data this session — see the next queue step for that verification pass. |

...
| 96 | Change Monitor CM-01 (**AG-42**, renumbered from AG-30 on 2026-08-02) | NOT-BUILT | Per `AGENT_VERIFICATION_LOG.md` "AG-30" (July 30 2026): confirmed zero code exists anywhere — no file, no class, no route, no registry entry, for this concept (change detection triggering re-enrichment). **Renumbered 2026-08-02** (`AGENTS_v2.md` §1.4): this document's own "AG-30" number was contradictorily claimed by two unrelated concepts (this phantom one and the real Donor Intent Monitor — row #218). Resolved by moving this never-built spec to AG-42, leaving AG-30 permanently and unambiguously Donor Intent Monitor. Do not reuse AG-30 for this concept again. |
...
| 132 | Forecast Agent (AG-26) | BUILT — UNVERIFIED | `src/lib/agents/funding-forecast-agent.ts`, class `FundingForecastAgent extends AutonomousAgent`, `agentId: "ag-26-forecast"` (enum value added live via migration 110, confirmed present in the live `agent_type` enum both by direct `psql` query and the PostgREST OpenAPI schema). Implements the spec's full numbered process: deterministic probability-weighted projection per period (reusing `computeGrantProbability()`'s neutral-fallback convention where the spec cross-references it), zero-opportunity and unscored-opportunity branch logic, one bounded Claude call per org per run for the narrative layer with graceful degradation on Claude failure. Wired into `worker/scheduler.ts` as a real, dedicated monthly slot (1st of month, 4:00 AM CST) via `runFundingForecastMonthlyPipeline()` in `worker/autonomous-orchestrator.ts` — not folded into the 2AM sweep, per the spec's own fixed-clock-time trigger design. `pnpm tsc --noEmit` clean. Not yet live-execution-tested against real production data in this pass — see `AGENT_VERIFICATION_LOG.md` for the live-verify methodology used for AG-10/AG-17/etc.; that pass has not been run for AG-26 yet. |
...
| 141 | Simulation Agent | PLANNED | Agent **AG-41** (renumbered from AG-28 on 2026-08-02 — AG-28 is now permanently AG-28 Autonomous Follow-Up Generator, row #202; see `AGENTS_v2.md` §1.4). What-if modeling. Phase 4. Not to be confused with the real, different, already-BUILT AG-37 Simulation Agent in `AGENTS_v2.md`'s Phase 2-5 section. |
...
| 218 | AI Donor Intent Engine (**AG-30, confirmed permanent 2026-08-02**) | BUILT — BLOCKED | `donor-intent-monitor-agent.ts` (`agentId: "ag-30-donor-intent"`) + `/api/intelligence/donor-intent` + page. **Enum gap fixed live 2026-08-02** (`fix-agent-type-enum-gap.sql` applied via `DATABASE_URL`/psql, confirmed via the live PostgREST OpenAPI schema) — the `22P02` failure this row previously described no longer occurs. Also fixed the same day: `loadOrgProfile()` was querying a nonexistent `organizations.service_areas` column, corrected to the real `service_area`. Re-verified live: the agent now completes a real run (`status: completed`) instead of crashing — but still produces zero real signals because `corporate_prospects` doesn't exist in production, reported as a caught error rather than a crash. See `AGENT_VERIFICATION_LOG.md`'s enum-gap entries for full evidence. **AG-30 is now this agent's permanent, sole number** — the conflicting phantom "Change Monitor Agent (CM-01)" that used to also claim AG-30 (row #96) was renumbered to AG-42 the same day; do not reuse AG-30 for anything else. |

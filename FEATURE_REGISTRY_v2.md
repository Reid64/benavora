...
| 133 | Forecast Dashboard | NOT-BUILT | `/reports/forecast` confirmed absent as a UI page — searched `src/app/(dashboard)` for any `forecast`-named page, zero results. The backing API route now exists (`src/app/api/reports/forecast/route.ts`, added 2026-08-07, GET+POST, real data confirmed) — row #133 is specifically the missing dashboard page consuming it, not the data layer. Consistent with PLANNED, restated as NOT-BUILT for clarity. |

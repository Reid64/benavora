# TARRITRIX 1.0 — AGENTS.md DELTA SPECIFICATION
...
```
### Phase 1 Agents (15) — ⏳ NOT STARTED (1 SHIPPED 2026-05-18 as of governance snapshot)

Listed in execution sequence order per BLUEPRINT.md Section 4.2 Post-Onboarding Pipeline:

- A-01 Intake Processor ✅ SHIPPED 2026-05-18 (commit 3a0a931)
- A-44 Client Knowledge Ingestion Engine — relocated to Phase 1 per 2026-05-23 governance synchronization (was Phase 1.5)
- A-10 Content Profile Builder (with heatmap)
- A-02 Page Generator — blocked from execution per Contract 73 until A-44 produces successful current ingestion version for the target client
- A-03 Schema Generator
- A-04 Map Embed Generator
- A-05 Page Validator (15 gates)
- A-06 Internal Link Builder
...
6. A-02 entry point updated to enforce Contract 73 check
7. Then remaining Phase 1 agents per existing dependency chain
...
**Next Priority:** RBAC foundation (Migrations N+1 through N+8 plus permission-matrix.ts + role-context.ts library) — A-01 Intake Processor is already shipped; the next agent build is A-44, which depends on RBAC foundation being in place first because A-44 will write to client_ingestion_versions which is one of the RBAC migration's new tables.
```

### 3.4 Note on A-01 already-shipped status

Per the existing AGENTS.md content, A-01 Intake Processor shipped 2026-05-18 (verified in STATE_OF_THE_BUILD.md). The original "Phase 1 Agents (14) — NOT STARTED" header was already slightly stale at the moment of governance authoring. We are correcting it to acknowledge the shipped state AND updating the count to 15 with A-44 added.
...
This document requires operator sign-off before AGENTS.md is modified in the repository.

**Operator approval format:**

- **"Approved — proceed to Document 6"** to advance to BEHAVIORAL_CONTRACTS.md delta updates (Contracts 71, 72, 73; Contract 67 amendment).
- **"Edits required: [list]"** to request specific revisions to this delta before approval.

After all 8 documents are approved, the CC prompt that performs the synchronized governance commit will apply this delta to AGENTS.md as one of seven file modifications in a single atomic operation.

---

**End of Document 5 of 8.**

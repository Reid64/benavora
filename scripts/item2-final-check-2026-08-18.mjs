...
(async () => {
  ...
  const [resp] = await Promise.all([
    page.waitForResponse((r) => r.url().includes("/api/agents/grants-gov") && r.request().method() === "POST", { timeout: 60000 }),
    page.locator('button:has-text("Run Now")').first().click(),
  ]);
  console.log("grants-gov POST status:", resp.status());
  await page.waitForTimeout(1000);
  await page.screenshot({ path: "smoke-test-output/ITEM2-FINAL-grantsgov-2026-08-18.png", fullPage: true });

  const link = page.locator('a:has-text("View Opportunities")');
  console.log("View Opportunities link count:", await link.count());

...
async function main() {
  ...
  console.log("category select bg:", bg, "border:", border, "text color:", color);

  // category filter functional check
  await select.selectOption({ label: "Private Foundation" }).catch(async () => {
    const opts = await select.locator("option").allTextContents();
    console.log("options:", opts);
  });
  await page.waitForTimeout(500);
  const cardCount = await page.locator('div[role="button"]').count();
  console.log("cards after category filter:", cardCount);

// ============================================================================
// BENAVORA — Intelligence Library corpus seed data + seeding logic
//
// Extracted from scripts/seed-intelligence-corpus.ts so the 20 hand-written
// proposal records and the dedup/insert logic have one source of truth,
// shared by both the standalone `pnpm seed:intelligence` script and
// scripts/populate-all-data.ts.
//
...
export const ALL_PROPOSALS: SeedProposal[] = [
  ...NIH_PROPOSALS,
  ...HUD_PROPOSALS,
  ...DOJ_PROPOSALS,
  ...USDA_PROPOSALS,
...
export async function seedIntelligenceCorpus(supabase: SupabaseClient): Promise<void> {
  ...
  if (newRows.length === 0) {
    console.log(
      `  All ${ALL_PROPOSALS.length} seed records already exist (matched by source_url). Nothing to insert.`,
    );
    return;
  }

...
// pass on /foundations, plus the fixed-teal (#22D3EE) "Import as Funder" CTA
// treatment. Reuses the magic-link login technique established in
// verify-section-accents-2026-08-15.mjs / verify-nonprofits-accent-2026-08-16.mjs.
// Run with an argument of "before" or "after" to pick the screenshot filename;
// CTA/accent-element checks only run in "after" mode.
...
async function main() {
  ...
  try {
    await page.goto(BASE_URL + "/foundations", { waitUntil: "load", timeout: 45000 });
    await page.waitForTimeout(3000);
    log("loaded", page.url().includes("/foundations"), page.url());

    await page.screenshot({
      path: `smoke-test-output/foundations-accent-${MODE}-2026-08-16.png`,
      fullPage: true,
    });
    ...
    const titleText = await page.evaluate(() => {
      ...
      const t = nodes.find((d) => d.textContent?.trim() === "Foundation Directory");
      return t ? t.textContent : null;
    });
    log("title-present", titleText === "Foundation Directory", String(titleText));

    // Foundation cards or empty state present (real data check).
    const cardCount = await page.evaluate(() => {
      return Array.from(document.querySelectorAll("button")).filter(
        (b) => b.textContent?.trim() === "Import as Funder",
      ).length;
    });
    log("foundation-cards-present", cardCount > 0, `import-buttons=${cardCount}`);

    // Search + filter bar intact.
    const formOk = await page.evaluate(() => {
      const input = document.querySelector('input[aria-label="Search foundations"]');
      return !!input;
    });
    ...
    if (MODE === "after") {
      ...
      // 1. StatCard top-border + value accent.
      const statAccent = await page.evaluate(() => {
        const ps = Array.from(document.querySelectorAll("p"));
        const label = ps.find((p) => p.textContent?.trim() === "Foundations");
        const card = label ? label.parentElement : null;
        const value = card ? card.querySelector("p:nth-of-type(2)") : null;
        return {
          borderTop: card ? getComputedStyle(card).borderTopColor : null,
          valueColor: value ? getComputedStyle(value).color : null,
        };
      });
      log(
        "statcard-accent",
        statAccent.borderTop === ACCENT && statAccent.valueColor === ACCENT,
        JSON.stringify(statAccent),
      );

      // 2. Search input focus ring color (border-focus class uses accent).
      ...
      // 3. "Import as Funder" CTA buttons are fixed teal.
      const importBtnColor = await page.evaluate(() => {
      ...
      log(
        "import-cta-teal",
        !!importBtnColor && importBtnColor.bg === TEAL,
        JSON.stringify(importBtnColor),
      );

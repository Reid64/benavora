...
// intelligence patterns. knowledge_patterns has no unique constraint (it is
// shared, cross-org intelligence library data — see 096_knowledge_engine.sql
// header), so idempotency is handled here by checking pattern_description
// against what's already seeded and skipping duplicates.
//
//   pnpm seed:patterns
// ============================================================================
...
const ALL_PATTERNS: KnowledgePattern[] = [
  ...NARRATIVE_PATTERNS,
  ...TIMING_PATTERNS,
  ...BUDGET_PATTERNS,
  ...FUNDER_PATTERNS,
...
async function fetchExistingDescriptions(): Promise<Set<string>> {
  ...
  for (;;) {
    ...
    if (error) {
      fatal(`could not query knowledge_patterns: ${error.message}`);
    }
...
async function main() {
  ...
  if (ALL_PATTERNS.length !== 30) {
    fatal(`expected exactly 30 patterns, built ${ALL_PATTERNS.length}`);
  }

  const existing = await fetchExistingDescriptions();
  ...
  const { error } = await admin.from("knowledge_patterns").insert(toInsert);
  if (error) {
    fatal(`insert failed: ${error.message}`);
  }

    ...
    <div className="flex items-center gap-2 mb-8">
      {steps.map((s, i) => (
        <div key={s.id} className="flex items-center gap-2">
          ...
          <span
            className={`text-sm ${
              i === activeIndex ? "text-gray-200" : i < activeIndex ? "text-gray-400" : "text-gray-600"
            }`}
          >
            {s.label}
          </span>
          {i < steps.length - 1 && <div className="w-8 h-px bg-gray-800 mx-1" />}
        </div>
      ))}
    </div>
  );
  ...
  if (step === "upload") {
    return (
      <div className="max-w-2xl mx-auto py-8 px-4">
        <div className="mb-6">
          <Link href="/funders" className="text-sm text-gray-400 hover:text-gray-200 transition-colors">
            &larr; Back to Funders
          </Link>
          <h1 className="text-2xl font-semibold text-white mt-2">Import Funders from CSV</h1>
    ...
    return (
      <div className="max-w-2xl mx-auto py-8 px-4">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-white mt-2">Map Columns</h1>
          <p className="text-gray-400 mt-1">
            Match your CSV columns to funder fields.{" "}
            <span className="text-gray-500 text-sm font-mono">{csvFile?.name}</span>
          </p>
        </div>
            ...
            {FUNDER_FIELDS.map((field) => (
              <div key={field.key} className="flex items-start gap-4">
                ...
                <select
                  ...
                  className="flex-1 bg-gray-800 border border-gray-700 text-gray-200 text-sm rounded-md px-3 py-2 focus:outline-none focus:border-blue-500 transition-colors"
                >
    ...
    return (
      <div className="max-w-5xl mx-auto py-8 px-4">
        <div className="mb-6">
          ...
          <h1 className="text-2xl font-semibold text-white mt-2">Preview Import</h1>
          <p className="text-gray-400 mt-1">
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800/50">
                {previewRows.map((row, rowIdx) => (
                  <tr key={rowIdx} className="hover:bg-gray-800/20 transition-colors">
                    {mappedFields.map((f) => {
                      ...
                      return (
                        <td
                          key={f.key}
                          className="px-4 py-2.5 text-gray-300 max-w-xs truncate"
                        >
                          {val || (
  ...
  if (step === "importing") {
    return (
      <div className="max-w-2xl mx-auto py-24 px-4 text-center">
        <div
          className="h-10 w-10 rounded-full border-2 border-gray-700 border-t-blue-400 animate-spin mx-auto mb-5"
          aria-hidden="true"
        />
        <p className="text-gray-300 text-lg font-medium">Importing funders&hellip;</p>
      ...
      <div className="max-w-2xl mx-auto py-8 px-4">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-lg p-6">
          ...
          <div className="mt-6 pt-5 border-t border-gray-800 flex gap-3">
            ...
            <button
              onClick={handleReset}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium bg-gray-800 text-gray-300 hover:bg-gray-700 hover:text-white transition-colors"
            >
              Import Another File
            </button>
          </div>

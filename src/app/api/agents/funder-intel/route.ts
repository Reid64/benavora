...
import { FunderIntelAgent } from "@/lib/agents/funder-intel";

// POST /api/agents/funder-intel
//
...
// the FunderIntelAgent which fetches the funder website, sends content to
// Claude for extraction, and upserts the result into funder_intelligence.
//
...
export async function POST(request: Request) {
  ...
  if (isRateLimited(organizationId)) {
    return jsonError(
      "Too many funder intelligence runs. Please wait a moment.",
      "rate_limited",
      429,
    );
  }
  ...
  try {
    const agent = new FunderIntelAgent({
      client: supabase,
      organizationId,
      triggeredBy,
      model,
    ...
    return jsonError(
      err instanceof AgentError
        ? err.message
        : "Funder intelligence run failed. Please try again.",
      err instanceof AgentError ? err.code : "funder_intel_failed",
      status,
    );
  }
}

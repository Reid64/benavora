...
// Grant Summary endpoint (AGENTS.md Agent 01). Authenticates the user, derives
// organization_id from their profile (never the request body), and runs the
// GrantSummaryAgent, which fetches/extracts the opportunity, patches its
// structured fields, logs to agent_runs, and tracks token usage.
...
export async function POST(request: Request) {
  ...
  const { opportunityId, rawText } = (body ?? {}) as {
  ...
  if (rawText !== undefined && typeof rawText !== "string") {
    return jsonError("rawText must be a string.", "invalid_input", 400);
  }
  ...
  try {
    ...
    return jsonError(
      "Opportunity summarization failed. Please try again.",
      "summarize_failed",
      500,
    );
  }

...
function validateProposal(raw: RawProposal): ValidatedProposal | null {
  if (!isProposalType(raw.proposal_type)) return null;
...
export class SelfImprovementAgent {
  ...
  private async calculateYesterdayMetrics(): Promise<{
  ...
  private async identifyUnderperformers(): Promise<UnderperformingAgent[]> {
    ...
    const { data, error } = await this.supabase
      .from("agent_performance_metrics")
      .select(
        "agent_id, runs_total, runs_successful, avg_confidence_score, decisions_requiring_review",
      )
    ...
    for (const [agentId, agg] of aggregates) {
      ...
      const avgConfidenceScore =
        agg.confidenceWeight > 0
          ...
          : null;
      const reviewRatio =
        agg.runsTotal > 0 ? agg.requiringReview / agg.runsTotal : null;

      const reasons: string[] = [];
      if (successRate !== null && successRate < SUCCESS_RATE_THRESHOLD) {
        reasons.push(
          `success rate ${(successRate * 100).toFixed(1)}% < ${SUCCESS_RATE_THRESHOLD * 100}%`,
        );
      }
      ...
      if (reviewRatio !== null && reviewRatio > REVIEW_RATIO_THRESHOLD) {
        reasons.push(
          `${(reviewRatio * 100).toFixed(1)}% of decisions required human review`,
        );
      }
  ...
  private async generateProposals(
    ...
    const underperformerLines = underperformers.map(
      (agent) =>
        `- ${agent.agentId} (${agent.runsTotal} runs over trailing ${METRICS_LOOKBACK_DAYS}d): ${agent.reasons.join("; ")}`,
    );
    ...
    const system =
      "You are the Benavora platform's Autonomous Continuous Improvement Engine " +
...
If there is not enough data to propose anything with reasonable confidence, return an empty array [].`;
    ...
    for (const entry of raw) {
      ...
      if (proposal) validated.push(proposal);
    }
    return validated;
  }
  ...
  private async insertProposals(
    ...
    const toInsert = proposals.filter(
      (proposal) => proposal.confidence_score >= MIN_CONFIDENCE_TO_PROPOSE,
    );
  ...
  private async notifyPlatformOwners(
    highRiskProposals: ValidatedProposal[],
  ): Promise<void> {
    if (highRiskProposals.length === 0) return;
    ...
    const message =
      highRiskProposals.length === 1 && highRiskProposals[0]
  ...
  async run(triggerSource: TriggerSource): Promise<AutonomousAgentResult> {
    ...
    try {
      ...
      return {
        ...
        decisions: [],
        nextActions:
          highRisk.length > 0 ? ["platform_owners_notified_high_risk"] : [],
        errors: [],
      };
    } catch (err) {

...
export interface ConfirmationData {
  ...
  contact_for_questions: string | null;
}
...
const EMPTY_CONFIRMATION: ConfirmationData = {
...
export async function parseConfirmationPage(page: unknown): Promise<ConfirmationData> {
  const p = page as {
...
${pageText.slice(0, 6000)}`,
  ...
  try {
    ...
    if (!jsonMatch) return { ...EMPTY_CONFIRMATION };

    const parsed = JSON.parse(jsonMatch[0]) as Partial<ConfirmationData>;

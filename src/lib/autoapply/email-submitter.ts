...
function buildSubject(organizationName: string, requestType: string): string {
...
function buildAmountLine(requestType: string, askAmount?: number): string {
  if (requestType !== "monetary" || askAmount == null) return "";
...
function buildRequestTypeLabel(requestType: string): string {
...
function buildHtmlBody(params: EmailSubmissionParams): string {
  const {
    funderName,
    contactName,
    organizationName,

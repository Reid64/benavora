...
function isSharedPlatform(domain: string): boolean {
  return SHARED_PLATFORMS.some((p) => domain === p || domain.endsWith(`.${p}`));
}

export interface CrossClientDedupResult {
  blocked: boolean;
  reason?: string;
  delayUntil?: Date;
}

export interface DomainThrottleResult {
  blocked: boolean;
  reason?: string;
}
...
export interface ControlsReport {
  ...
  dailyLimit: number;
  domainsToday: string[];
  crossClientBlocksActive: number;
}

export class SubmissionControls {
  /**
   ...
   */
  async checkCrossClientDedup(
    funderDomain: string,
    orgId: string,
    supabase: any,
  ): Promise<CrossClientDedupResult> {
   ...
   */
  async checkDomainThrottle(
    portalUrl: string,
    supabase: any,
  ): Promise<DomainThrottleResult> {
    ...
    const funderIds = ((funderData ?? []) as Array<{ id: string }>).map((f) => f.id);
    ...
    const submissionCount = (count ?? 0) as number;
  ...
  async checkVelocityLimits(
    ...
    const tier =
      (orgData as { subscription_tier: string | null } | null)?.subscription_tier ?? 'starter';
    ...
    const dailyCount = (count ?? 0) as number;
   ...
   */
  async getControlsReport(orgId: string, supabase: any): Promise<ControlsReport> {
    // --- daily count + limit ---
    ...
    const dailyCount = (dailyCountRaw ?? 0) as number;
    const tier =
      (orgData as { subscription_tier: string | null } | null)?.subscription_tier ?? 'starter';
    const dailyLimit = DAILY_LIMITS[tier] ?? DAILY_LIMITS['starter'] ?? 10;
    ...
    const funderIdsToday = [
      ...new Set(
        ((submissionsToday ?? []) as Array<{ funder_id: string | null }>)
          .map((s) => s.funder_id)
          .filter((id): id is string => id !== null),
      ),
    ];

    let domainsToday: string[] = [];
    if (funderIdsToday.length > 0) {
      ...
      domainsToday = [
        ...new Set(
          ((funderData ?? []) as Array<{ giving_portal_url: string | null }>)
    ...
    const crossClientBlocksActive = new Set(
      ((blockData ?? []) as Array<{ funder_domain: string }>).map((r) => r.funder_domain),
    ).size;

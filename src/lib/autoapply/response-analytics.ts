...
export interface FunderResponseStats {
  funderId: string;
  funderName: string;
  averageDays: number;
...
export interface CategoryResponseStats {
  category: string;
  averageDays: number;
  totalSubmissions: number;
  totalResponses: number;
...
export interface ResponseTimeReport {
  ...
  byFunder: FunderResponseStats[];
  byCategory: CategoryResponseStats[];
  overdue: OverdueSubmission[];
}
...
export interface ROIReport {
  ...
  costPerFundedDollar: number | null;
  totalSubmissions: number;
...
interface SubmissionRow {
  ...
  funders: { name: string; category: string | null } | null;
}
...
export async function getResponseTimeStats(
  ...
  const { data: allRows } = (await supabase
    ...
    .not('submitted_at', 'is', null)) as { data: SubmissionRow[] | null };

  const rows: SubmissionRow[] = allRows ?? [];
  ...
  const allDays = responded.map((r) =>
    daysBetween(r.submitted_at as string, r.response_received_at as string),
  );

  const overall: OverallResponseStats = {
  ...
  const funderMap = new Map<
    string,
    { name: string; days: number[]; totalSubmissions: number }
  >();
  ...
  const byFunder: FunderResponseStats[] = Array.from(funderMap.entries()).map(
    ([funderId, info]) => ({
      funderId,
  ...
  const categoryMap = new Map<
    string,
    { days: number[]; totalSubmissions: number }
  >();
  ...
  const byCategory: CategoryResponseStats[] = Array.from(categoryMap.entries()).map(
    ([category, info]) => ({
      category,
      averageDays: average(info.days),
      totalSubmissions: info.totalSubmissions,
  ...
  const funderAvgMap = new Map<string, number>();
  for (const f of byFunder) {
    if (f.totalResponses > 0) funderAvgMap.set(f.funderId, f.averageDays);
  }
  ...
  for (const r of pending) {
    const fid = r.funder_id ?? 'unknown';
    const avgForFunder =
      funderAvgMap.get(fid) ?? (overall.averageDays > 0 ? overall.averageDays : 30);
    const threshold = avgForFunder * 1.5;
    const daysElapsed = (now - new Date(r.submitted_at as string).getTime()) / (1000 * 60 * 60 * 24);
...
export async function getChannelComparison(
  ...
  const channelMap = new Map<
    string,
    { total: number; success: number; responseDays: number[] }
  >();
  ...
  for (const r of rows) {
    ...
    if (confirmed || responded) entry.success += 1;
  ...
  const channels: ChannelMetrics[] = Array.from(channelMap.entries()).map(
    ([channel, info]) => ({
      channel,
      ...
      averageResponseDays:
        info.responseDays.length > 0 ? average(info.responseDays) : null,
    }),
  );
...
export async function getConversionFunnel(
  ...
  const queueCount: number = (queueRes as { count: number | null }).count ?? 0;
  ...
  const agreements: AgreementRow[] = agreementsRes.data ?? [];
  // Only count agreements linked to an autoapply submission
  const submissionIds = new Set(submissions.map((s) => s.id));
  const fundedCount = agreements.filter(
    (a) => a.submission_id !== null && submissionIds.has(a.submission_id),
  ).length;
  ...
  const stages: FunnelStage[] = stageNames.map((stage, i) => {
    ...
    const dropOffRate =
      prev !== null && prev > 0 ? ((prev - count) / prev) * 100 : null;
    const retentionRate =
      prev !== null && prev > 0 ? (count / prev) * 100 : null;
    return { stage, count, dropOffRate, retentionRate };
  });

  const ultimateConversionRate =
    queueCount > 0 ? (fundedCount / queueCount) * 100 : 0;
...
export async function getROICalculation(
  orgId: string,
  supabase: any,
): Promise<ROIReport> {
  ...
  const breakdown: ROIBreakdown = usageRows.reduce(
    (acc, r) => ({
  ...
  const totalSubmissions: number =
    (submissionsCountRes as { count: number | null }).count ?? 0;

  const roi = totalCost > 0 ? ((totalFunded - totalCost) / totalCost) * 100 : null;
  const costPerSubmission =
    totalSubmissions > 0 ? totalCost / totalSubmissions : null;

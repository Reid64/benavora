...
const LAND_BANK_USER_AGENT =
  "BenavoraBot/1.0 (+https://benavora.vercel.app/security; Donor Discovery land bank directory scrape)";

export class LandBankDirectoryError extends Error {
...
type LandBankField = "org_name" | "state" | "city" | "website" | "contact";

const HEADER_ALIASES: Record<string, LandBankField> = {
  "name": "org_name",
...
function cellLinkHref(el: HTMLElement): string | null {
  const anchor = el.querySelector("a[href]");
  const href = anchor?.getAttribute("href")?.trim();
  return href && !href.startsWith("mailto:") ? href : null;
}

function cellMailto(el: HTMLElement): string | null {
...
function findDirectoryTable(root: HTMLElement): HTMLElement | null {
  ...
  for (const table of tables) {
    ...
    if (hasName && hasLocation) return table;
  }
  return null;
}

function mapColumnIndexes(headerCells: HTMLElement[]): Partial<Record<LandBankField, number>> {
  const map: Partial<Record<LandBankField, number>> = {};
...
function cellAt(cellElements: HTMLElement[], index: number | undefined): HTMLElement | undefined {
  return index !== undefined ? cellElements[index] : undefined;
}

function cellsToRecord(
  cellElements: HTMLElement[],
  columnIndex: Partial<Record<LandBankField, number>>,
): LandBankRecord | null {
  ...
  const website =
    (websiteCell ? cellLinkHref(websiteCell) || cellText(websiteCell) || null : null) ??
 ...
 */
export function parseLandBankDirectory(html: string): LandBankRecord[] {
  ...
  if (columnIndex.org_name === undefined) return [];

  const records: LandBankRecord[] = [];
...
async function fetchDirectoryHtml(): Promise<string> {
  const result = await fetchCompliant(DIRECTORY_URL, {
...
function buildHqAddress(city: string | null, state: string | null): string | null {
  const parts = [city, state].filter((p): p is string => Boolean(p));
  return parts.length > 0 ? parts.join(", ") : null;
}

async function upsertLandBankRecord(record: LandBankRecord): Promise<RawProspect | null> {
  try {
 ...
 */
export async function fetchLandBankDirectory(): Promise<RawProspect[]> {
  const html = await fetchDirectoryHtml();
  const records = parseLandBankDirectory(html);

  if (records.length === 0) {
    console.warn(
      "[land-bank-adapter] Zero land bank rows parsed — either the directory is genuinely empty " +

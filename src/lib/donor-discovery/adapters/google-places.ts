...
import { warnIfMissingOptionalEnv } from "@/lib/env";
import { upsertDirectoryRecord } from "@/lib/donor-discovery/directory";
...
// ── NAICS → Places API (New) mapping table ──────────────────────────────────
//
// `includedType` values are Places API (New) primary types — see
// https://developers.google.com/maps/documentation/places/web-service/place-types
// Only codes with a confidently-correct type get one; entries below without
// an `includedType` still search fine on `textQuery` alone, which is the
// required field. Verify types against Google's published list before
// leaning on `includedType` for anything precision-critical — Places
// silently ignores a `textQuery` type mismatch, but rejects an outright
// invalid type string with a 400.
//
// To extend: add a 6-digit NAICS code. No code elsewhere needs to change —
// codes absent from this table fall back to the taxonomy label (see
// `resolveSearchTerms`).
interface NaicsPlacesEntry {
  includedType?: string;
  textQuery: string;
}

const NAICS_PLACES_MAP: Record<string, NaicsPlacesEntry> = {
...
export interface RadiusGeography {
  center: { lat: number; lng: number };
  radius_mi: number;
}

/** Mirrors the `geography` jsonb shapes in DONOR_DISCOVERY_ARCHITECTURE.md §3. */
export type DdGeography = RadiusGeography | { states: string[] } | { national: true };
...
export interface DonorDirectoryUpsertShape {
  legal_name: string;
  website: string | null;
...
export interface EnumerateResult {
  prospects: DonorDirectoryUpsertShape[];
  directoryIds: string[];
  requestsMade: number;
  estCostUsd: number;
}

export class DdBudgetExceededError extends Error {
  constructor(monthlyBudgetUsd: number, spentUsd: number) {
    super(
      `Donor Discovery Google Places budget exceeded: $${spentUsd.toFixed(2)} spent of ` +
...
// ── Places API request/response shapes (the subset we use) ─────────────────
...
function requireRadiusGeography(geography: DdGeography): RadiusGeography {
  if ("center" in geography && "radius_mi" in geography) return geography;
  throw new Error(
    "google-places adapter requires a radius geography ({center, radius_mi}); " +
      "state/national geography is not supported by this adapter yet.",
  );
}

// ── NAICS → search term resolution ──────────────────────────────────────────
...
async function resolveSearchTerms(naicsCodes: string[]): Promise<SearchTerm[]> {
  ...
  if (unmapped.length > 0) {
    ...
    const labelsByCode = new Map<string, string>(
      error || !data ? [] : data.map((row: { code: string; label: string }) => [row.code, row.label]),
    );
    ...
    for (const code of unmapped) {
      ...
      if (label) {
        ...
        console.warn(
          `[google-places] No NAICS mapping or taxonomy label found for code "${code}" — skipping.`,
        );
      }
    }
  }
...
async function assertBudgetAvailable(monthKey: string): Promise<void> {
  const supabase = createAdminClient();
  ...
  const spentUsd = (data as { est_cost_usd: number } | null)?.est_cost_usd ?? 0;
...
async function recordSpend(monthKey: string): Promise<void> {
  const supabase = createAdminClient();
...
export function normalizePlace(
  ...
  naicsCode: string,
): DonorDirectoryUpsertShape | null {
...
async function upsertToDirectory(prospect: DonorDirectoryUpsertShape): Promise<string | null> {
  try {
...
async function searchTextPage(
  ...
  if (term.includedType) body.includedType = term.includedType;
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Places API Text Search failed (${response.status}) for "${term.textQuery}": ${errorBody.slice(0, 500)}`,
    );
  }
 ...
 */
export async function enumerate(params: EnumerateParams): Promise<EnumerateResult> {
  const apiKey = process.env.GOOGLE_PLACES_API_KEY;
  if (!apiKey) {
    warnIfMissingOptionalEnv("GOOGLE_PLACES_API_KEY");
    throw new Error(
      "GOOGLE_PLACES_API_KEY is not set — cannot run the Google Places registry adapter.",
    );
  }

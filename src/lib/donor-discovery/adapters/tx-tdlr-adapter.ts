...
import type { Geography, RawProspect, RegistryAdapter } from "@/lib/donor-discovery/adapters/google-places-adapter";
...
const TDLR_USER_AGENT =
  "BenavoraBot/1.0 (+https://benavora.vercel.app/security; Donor Discovery TX TDLR licensee search)";

export type TdlrLicenseType = "ELEC" | "PLMB" | "HVAC" | "ELEV" | "BLRP";

export const TDLR_LICENSE_TYPES: TdlrLicenseType[] = ["ELEC", "PLMB", "HVAC", "ELEV", "BLRP"];

/** See file header — ELEV and BLRP share 238290 per the Census NAICS manual's
 * "Other Building Equipment Contractors" grouping. */
export const NAICS_BY_LICENSE_TYPE: Record<TdlrLicenseType, string> = {
  ELEC: "238210",
  PLMB: "238220",
  HVAC: "238220",
  ELEV: "238290",
...
export class TdlrError extends Error {
...
export interface TdlrLicenseeRecord {
  business_name: string;
  license_number: string;
  license_type: TdlrLicenseType;
  city: string | null;
  zip: string | null;
  phone: string | null;
  /** ISO yyyy-mm-dd, or null if the results table didn't carry a parseable date. */
...
function licenseTypesForNaics(naicsCodes: string[]): TdlrLicenseType[] {
  const wanted = new Set(naicsCodes);
  return TDLR_LICENSE_TYPES.filter((licenseType) => wanted.has(NAICS_BY_LICENSE_TYPE[licenseType]));
}

// ── HTML parsing ─────────────────────────────────────────────────────────────

type LicenseeField = "business_name" | "license_number" | "city" | "zip" | "phone" | "expiration_date";

const HEADER_ALIASES: Record<string, LicenseeField> = {
...
function findResultsTable(root: HTMLElement): HTMLElement | null {
...
function mapColumnIndexes(headerCells: HTMLElement[]): Partial<Record<LicenseeField, number>> {
  const map: Partial<Record<LicenseeField, number>> = {};
...
function cellsToRecord(
  cells: string[],
  columnIndex: Partial<Record<LicenseeField, number>>,
  licenseType: TdlrLicenseType,
): TdlrLicenseeRecord | null {
 ...
 * changed" warning.
 */
export function parseLicenseeResults(html: string, licenseType: TdlrLicenseType): TdlrLicenseeRecord[] {
  ...
  if (columnIndex.business_name === undefined) return [];

  const records: TdlrLicenseeRecord[] = [];
 ...
 * assumption. */
export function isActiveLicense(record: TdlrLicenseeRecord, now: Date = new Date()): boolean {
...
async function fetchLicenseeSearchHtml(licenseType: TdlrLicenseType): Promise<string> {
  const url = new URL(TDLR_LOOKUP_URL);
  url.searchParams.set("SearchType", "Business");
  url.searchParams.set("LicenseType", licenseType);

  const result = await fetchCompliant(url.toString(), {
...
function buildHqAddress(city: string | null, zip: string | null): string | null {
  const cityState = city ? `${city}, TX` : null;
  const parts = [cityState, zip].filter((p): p is string => Boolean(p));
  return parts.length > 0 ? parts.join(" ") : null;
}

async function upsertLicenseeRecord(record: TdlrLicenseeRecord): Promise<RawProspect | null> {
  try {
    const directoryRecord = await upsertDirectoryRecord({
      ...
      geo: null,
      phone: record.phone,
      naics_codes: [NAICS_BY_LICENSE_TYPE[record.license_type]],
 ...
 */
export async function searchLicenseType(licenseType: TdlrLicenseType): Promise<RawProspect[]> {
  const html = await fetchLicenseeSearchHtml(licenseType);
  const records = parseLicenseeResults(html, licenseType);

  if (records.length === 0) {
    console.warn(
      `[tx-tdlr-adapter] Zero licensee rows parsed for license type "${licenseType}" — ` +
...
export const txTdlrAdapter: RegistryAdapter = {
  name: PROVIDER,
  enumerate,
};

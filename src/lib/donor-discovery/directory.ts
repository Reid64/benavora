...
export interface DirectoryRecordInput {
  ...
  /** Adapter identity, e.g. "google_places" — unioned into source_adapters. */
  source_adapter: string;
  /** §2B enrichment record. Existing non-null keys on file are never overwritten. */
...
function parseGeo(raw: unknown): DirectoryGeo | null {
  ...
  if (typeof raw === "object" && "x" in (raw as Record<string, unknown>)) {
...
function toDirectoryRecord(row: Record<string, unknown>): DirectoryRecord {
  return {
    id: row.id as string,
    legal_name: row.legal_name as string,
    dba_name: (row.dba_name as string | null) ?? null,
    naics_codes: (row.naics_codes as string[] | null) ?? [],
    civic_kind: (row.civic_kind as string | null) ?? null,
    website: (row.website as string | null) ?? null,
    hq_address: (row.hq_address as string | null) ?? null,
    ...
    source_adapters: (row.source_adapters as string[] | null) ?? [],
...
export async function upsertDirectoryRecord(
  ...
  return toDirectoryRecord(data as Record<string, unknown>);
}
...
export async function findOrCreateProspect(
  ...
  if (existing) {
    prospectId = (existing as { id: string }).id;
  } else {
    ...
    if (insertError) {
      ...
      if (retryError || !retry) {
        throw new Error(
          `prospect_insert_race_failed: ${retryError?.message ?? "row not found after race"}`,
        );
      }
      prospectId = (retry as { id: string }).id;
    } else {
  ...
  const { error: linkError } = await supabase
    ...
    .upsert(
      { prospect_id: prospectId, request_id: requestId },

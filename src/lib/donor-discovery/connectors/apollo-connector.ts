...
const apolloRateLimiter = new DomainRateLimiter(2_000);

export type ApolloConnectorErrorCode = "AUTH_FAILED" | "API_ERROR" | "NO_SEARCH_TARGET";

export class ApolloConnectorError extends Error {
  readonly code: ApolloConnectorErrorCode;
  constructor(code: ApolloConnectorErrorCode, message: string) {
    super(message);
...
function personName(person: ApolloPerson): string | null {
...
function toContact(person: ApolloPerson): DecisionMakerContact {
  return {
 ...
 */
async function searchByProspect(prospect: DirectoryRecord, apiKey: string): Promise<ApolloPerson[]> {
  ...
  throw new ApolloConnectorError(
    "NO_SEARCH_TARGET",
    `Directory record ${prospect.id} has neither a website nor a legal name to search Apollo by.`,
  );
}
...
async function enrich(prospect: DirectoryRecord, apiKey: string): Promise<ConnectorEnrichment> {
  ...
  const contacts = people.map(toContact).filter((contact) => isDecisionMakerTitle(contact.title));
...
export const apolloConnector: ConnectorEnricher = {
  provider: PROVIDER,
  enrich,
};

          ...
          <div key={stage.id} className="relative h-36 cursor-pointer perspective-1000 group">
            <div className="relative h-full w-full preserve-3d transition-transform duration-500 group-hover:rotate-y-180">
              ...
              <Link
                href={stage.href}
                className={cn(
                  "absolute inset-0 rounded-xl bg-white shadow-sm border border-slate-200 flex flex-col items-center justify-center backface-hidden",
                  stage.borderClass,
                )}
              >
                <Icon className="h-6 w-6 text-slate-500" aria-hidden />
                <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mt-2">
                  {stage.label}
                </p>

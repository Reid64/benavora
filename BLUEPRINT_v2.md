...
| The autonomous orchestrator had no execution log — a step's start/finish/outcome existed only in an in-memory `log: string[]` discarded at process exit, and nothing recorded whether a step's output actually passed evidence validation | FIXED (AR-6.2, 2026-09-17) | New `public.orchestration_logs` (migration 190, org-scoped via `organization_id`/`current_org_id()` RLS — no `company_id` anywhere in this schema): `schema_validation_passed`/`reconciliation_passed` booleans make a step's success claim falsifiable, `cost_log_id` joins `ai_usage_log` rather than duplicating a cost total. `src/lib/orchestration/orchestration-log.ts`'s `logOrchestrationStep()`/`runOrchestrationStep()` is the one typed writer (redacts API keys/tokens/passwords before insert), wired into `worker/autonomous-orchestrator.ts` at 25 boundaries covering 51/52 distinct step types — see `STATE_OF_THE_BUILD.md` "AR-6.2" for the full list and the one named gap (AG-38's dedicated cron entrypoint has no owning org by design). Migration applied live via the Supabase MCP connector, confirmed via `pg_policies`. `src/__tests__/integration/orchestration-logs.test.ts` 4/4 green including the real-DB RLS assertion. |
| Critical `alerts` rows had no delivery path (nowhere reaches a human), the model rate card table didn't exist, and no dashboard view existed for orchestration cost/health — spec called for an Edge Function, SQL-native Slack delivery, and a copy of a year-old rate card | FIXED (AR-6.4, 2026-09-17) | `pg_net`/`pg_cron`/`http` confirmed not installed and `supabase/functions` confirmed absent — delivery moved into the already-continuous Railway worker instead (`worker/alert-notifier.ts`, wired into `worker/index.ts`'s existing interval-loop set, same start/stop/waitForIdle shape as `stuck-run-watchdog.ts`). Polls `alerts` for `severity='critical' AND notified_at IS NULL`, posts a `redactSecrets()`-cleaned message to `FORGE_SLACK_WEBHOOK` (existing convention, no second webhook), sets `notified_at` only on a 2xx response so a failed post retries and a successful one never re-sends. `public.model_cost_reference` created (migration 192) — the spec's Sept-2025 `claude-opus-4`/`claude-sonnet-4` card was rejected (0 references anywhere in `src/`/`worker/`); seeded instead with the 5 models this codebase actually calls or could call next, rates verified live against `claude.com/pricing` on 2026-09-17 (not recalled from training — the live fetch caught that Sonnet 5's $2/$10 rate is still standard pricing, not an expired intro rate, before it could be seeded wrong). Five dashboard views (migration 193: `v_orchestration_run_summary`, `v_orchestration_daily_cost`, `v_alert_activity_summary`, `v_budget_utilization`, `v_agent_reliability`), all `security_invoker = true` (PG17 — a view defaults to definer semantics otherwise, silently bypassing RLS) and reading cost from `ai_usage_log`, not `orchestration_logs`. **Prerequisite gap found and fixed in the same migration:** `ai_usage_log` had RLS enabled but zero policies and zero `authenticated` grants — every view built on it would have been correctly safe but permanently empty for real users; added the same org-isolation SELECT policy `orchestration_logs` already has. See `STATE_OF_THE_BUILD.md` and `SCHEMA_REGISTRY_v2.md`'s "AR-6.4" sections for the full verification trail. |
| 5 corporate enrichment agents (`ea01_giving_detector`, `ea02_community_outreach_detector`, `ea05_career_page_analyzer`, `ea08_executive_biography_analyzer`, `ea09_contact_extractor`) — 215 combined failures live in `agent_runs` (86-88% failure rate each), all `browserType.launch: Executable doesn't exist at /root/.cache/ms-playwright/...` | **CODE FIXED, NOT YET LIVE (AR-7.1, 2026-09-17)** | `worker/Dockerfile` installs system `chromium` + sets `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD=1`; only 1 of 6 `chromium.launch()` call sites read the executable-path env var (`stealth-browser.ts`) — the other five, including `stealth-engine.ts` (which all 5 broken EA agents use), fell back to Playwright's empty default cache. Fixed with one shared `src/lib/browser/launch-chromium.ts` (`launchChromium()`/`resolveChromiumExecutablePath()`, resolution order: explicit option → `CHROMIUM_EXECUTABLE_PATH` env var (renamed from `PLAYWRIGHT_CHROMIUM_EXECUTABLE_PATH`, which was never a real Playwright variable) → known system chromium paths → Playwright's own default; throws naming every path tried if none resolve) — all 6 call sites now route through it. New FORGE gate `scripts/audit/forge-gates/ar-7-browser-launch-unified.mjs` blocks a future direct `chromium.launch()` call from reintroducing this. **NOT LIVE UNTIL REDEPLOY** — the Railway worker was not rebuilt/redeployed this session (explicit "DO NOT DEPLOY" instruction); all five agents remain broken in production until that happens, and this fix is unproven until a real post-redeploy `agent_runs` row shows `status='completed'` for one of them. See `STATE_OF_THE_BUILD.md`'s "AR-7.1" section. |
| `automation_sessions` rows with no terminal status block `SubmissionValidator.checkConcurrentAutomation()` for that org+funder forever — 7 rows stuck live (oldest 99 days), `autoapply_queue_processor` 32/32 runs failed on `concurrent_automation_conflict`, blocking AR-3.1's submit-integrity fix from ever executing | **PARTIALLY LIVE (AR-7.2, 2026-09-17)** | `processItem()`'s `finalizeAutomationSession()` call moved into its `finally` (was reachable only by convention before); `processBrowserAutomationItem()` gained a post-run guard that force-closes a session left in an unsettled status. New `reapStaleAutomationSessions()` sweep in `worker/stuck-run-watchdog.ts`, per-status thresholds (30m for `pending`/`in_progress`/`approved`, 7d for the human-wait `awaiting_approval` state). The 7 already-stuck rows were cleared **live** via migration `195_reap_stuck_automation_sessions.sql` (Supabase MCP), each raising a `manual_review_required` alert — non-terminal count verified 0 immediately after. `src/__tests__/integration/automation-session-lifecycle.test.ts` 4/4 green, including the assertion that `checkConcurrentAutomation()` flips from conflict to no-conflict after a reap. **Steps 1-2 (the code fix) are NOT LIVE UNTIL REDEPLOY** — same "DO NOT DEPLOY" constraint as AR-7.1; Steps 3-5 (the watchdog logic, applied once by hand, and the alerting) ARE live against production data. `autoapply_queue_processor` has not yet been observed to succeed — the lock is clear, not proven fixed end to end. See `STATE_OF_THE_BUILD.md`'s "AR-7.2" section. |
| **CORRECTION to the AR-6.2 row above.** AR-6.2's "wired at 25 boundaries covering 51/52 distinct step types" was true of the code but `orchestration_logs` had **zero rows, all time** when re-checked live on 2026-09-18, hours after that deploy, despite 184 real `agent_runs` in the prior 3 hours | FIXED (AR-9.1, 2026-09-18) | Two distinct causes, not one. **(1) Timing, self-resolving:** the one instrumented path (`worker/autonomous-orchestrator.ts`'s `agent_queue` consumer) is fed almost entirely by `runOrgPipeline()`'s once-daily nightly sweep — bursty (~4 minutes/day), not continuous. AR-6.2 deployed mid-gap; live-verified it fires and writes correctly once the next day's burst starts (29 real rows appeared during this session's investigation, from exactly the 16 documented step types, with zero code changes). **(2) Structural, was permanent until this fix:** the platform's actual highest-volume, continuous traffic — every `AutonomousAgent` subclass (`src/lib/agents/autonomous-base.ts`, 26 subclasses, incl. `ag-29-knowledge-indexer`'s 24/7 poll loop — 178 of the 184 measured runs), the AutoApply worker pipeline (`src/lib/autoapply/run-logger.ts`'s `withAgentRun()`), and the on-demand agent fleet (`src/lib/agents/base-agent.ts`) — all run entirely outside `worker/autonomous-orchestrator.ts` and were never wired to `logOrchestrationStep()`. Fixed by instrumenting all three shared run boundaries directly (not by touching the orchestrator), each writing exactly one row per real step and logging (not throwing, not swallowing) a `console.error("[orchestration_logs] WRITE FAILED/THREW ...")` if the write itself fails, so this can never again sit at zero unnoticed. New migration `196_orchestration_logs_authenticated_insert.sql` adds the `authenticated`+`current_org_id()`-scoped `INSERT` policy AR-6.2 explicitly omitted (correct at the time — no authenticated write path existed yet; `BaseAgent.run()` can now be invoked under a session client, same as `agent_runs`). **Migration 196 NOT YET APPLIED LIVE** — `DATABASE_URL`/`psql` (28P01) and the §11 Management API PAT (401) were both dead this session; does not block the primary fix, which runs under the service-role client (RLS-exempt) on both real traffic sources. See `STATE_OF_THE_BUILD.md`'s "AR-9.1" section for the full root-cause account, live before/after counts, and outstanding redeploy verification. |
